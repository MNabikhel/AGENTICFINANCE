...
Public pin stays `aether.protocol.1` / `0.96.0` until the pin would otherwise lie. `PROTOCOL.hosted` stays false. `WORLD_VERSION` stays 1. Catalog stays 120 rules unless a new first-deny is required.
...
### 201. An empty book is not a settlement photo

`clearing.settle_window` of a currency with zero open legs was an allow that photographed nothing, emptied nothing, and still minted a `win_` window object plus a `CLEARING_WINDOW` notary line claiming a settlement cycle ran. The docs promise a window "photographs open gross and empties the live book" — an empty settle does neither. The catalog's own precedents deny every other no-op after yes: a no-op freeze is `identity.freeze_state`, a second tombstone is `kya.revoke_state`. A second settle right after a first is guaranteed empty — a double photo of the same nothing. Coffer TAP is the bilateral cap (`clearing.bilateral_limit`). A system speaker stays `actor.system_scope`. A junior speaker stays `actor.role_capability`. A garbage currency stays `command.malformed` (400) before policy. The other currency's open legs are a different book. Money already moved at escrow — a window is the CCP photo, not a second payment.

- [x] A TAP (`pnpm demo film`) funds an $800 hire (one open USD leg), refuses `clearing.settle_window` of the empty USDC book as `clearing.settle_state` (a system speaker and a junior speaker still first-deny elsewhere; no window minted), then the funded USD book still settles once, the second photo right after is the same refuse, and that funded work still releases.
- [x] New first-deny: `clearing.settle_state`. Catalog 119 → 120. Protocol stays 0.96.0.

### 202. Next: only if the pin would lie

If an allow, a listed HTTP/MCP face, or the protocol card would lie, patch it. Do not mint 0.97 unless the pin would otherwise lie. Do not grind inspect overlays. Do not re-TAP .
