      ...
      return {
        tools: [
          ...
          })),
          {
            ...
            description: "Read-only runtime snapshot: agents, mandates, carts, payments, hires, KYA, clearing, audit head. Intents include derived live | expired | funded. Carts include derived live | expired | bound. Payments include derived live | expired | funded (a payment whose parent cart died is expired). RFQs include derived live | expired. Quotes include derived live | expired | spent | held (a hire quote in a dead room is expired).",
            inputSchema: { type: "object", properties: {} },
          },
          {
            name: "aether_get",
            description: "Fetch one object by id or alias (hid_, mid_, aid_, rid_, apd_, rfq_, qte_, dlg_, acct_, or cash account name). A qte_ quote includes derived status (live | expired | spent | held). Expired includes a lapsed FX validUntil and, for a hire quote, a dead parent RFQ. An FX quote is a window on the quote, not the room. A rfq_ room includes derived status (live | expired). A mid_ intent includes derived status (live | expired | funded). Funded is escrow-moved occupancy against this slip and wins over expired. A mid_ cart includes derived status (live | expired | bound). Bound is unique_payment occupancy and wins over expired. A mid_ payment includes derived status (live | expired | funded). Funded is escrow-moved occupancy and wins over expired. Expired includes a dead parent cart even when this check's exp still lives. A dlg_ hop includes derived status (live | expired | revoked).",
            inputSchema: {
              type: "object",
