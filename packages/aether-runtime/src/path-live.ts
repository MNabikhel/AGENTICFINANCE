...
import { PLATE_TLDR, analog } from "./story.js";
import { fundHire, mustDispatch, offerHire } from "./hire-flow.js";
import type { TapResult } from "./sprint-procurement.js";

export interface PathLiveScenario {
  id: string;
  clockStart: string;
  genesisNonce: string;
...
export interface PathLiveReport {
  ok: boolean;
  results: TapResult[];
...
export function loadPathLive(path: string): PathLiveScenario {
  return JSON.parse(readFileSync(path, "utf8")) as PathLiveScenario;
}
...
export function runPathLive(scenario: PathLiveScenario): PathLiveReport {
  ...
  const live = offerHire(rt, {
    ...
    seller: vendor.id,
    sku: "research.brief",
    spec: "an orphan hop is not a handshake",
    price,
    intentId,
  });
  ...
  const afterSneak = {
    denied: deniedRule(sneak, "kya.path_live"),
    floorDenied: deniedRule(floor, "kya.path_live"),
  ...
  const afterLegal = {
    plateAllows: allowedRule(legalAttempt, "kya.path_live"),
    pathAllows: allowedRule(legalAttempt, "kya.path_tighter"),
    tighterPlateAllows: allowedRule(tighterAttempt, "kya.path_live"),
  ...
  must(rt.dispatch(cmd("hire.deliver", vendor.id, { hireId, deliverable: { n: 1 } })), "deliver after plate deny");
  must(rt.dispatch(cmd("envelope.require", vendor.id, { hireId })), "require");
  must(
    rt.dispatch(cmd("envelope.submit", desk.id, { hireId, nonce: `nonce-${hireId}` })),
    "submit after plate deny",
  );
  ...
  const results: TapResult[] = [
    expect(
      hired.replayed !== true &&
        ...
        sizeBeforeSneak === 0,
      1,
      "a listed seller still funds a hire, with no incoming hop and no orphan hop written yet",
      hireId,
    ),
    expect(
      afterSneak.denied &&
        ...
        afterSneak.firstDeny === "kya.path_live" &&
        afterSneak.floorFirst === "kya.path_live" &&
        afterSneak.hops === hopsBeforeSneak &&
        afterSneak.size === sizeBeforeSneak &&
        afterSneak.funded === "funded",
      2,
      "minting a hop in another principal's name with no live incoming path is kya.path_live — not a grant wider than the incoming hop, not a nested parent hop, not whose name a handshake is in",
    ),
    expect(
      incoming.replayed !== true &&
        legal.replayed !== true &&
        tighter.replayed !== true &&
        afterLegal.plateAllows &&
        afterLegal.pathAllows &&
        afterLegal.tighterPlateAllows &&
        afterLegal.tighterAllows &&
        afterLegal.scoutHops === 1 &&
        afterLegal.vendorHops === 1 &&
        afterLegal.size === sizeBeforeSneak + 3 &&
        incomingId.startsWith("dlg_") &&
        legalId.startsWith("dlg_") &&
        tighterId.startsWith("dlg_"),
      3,
      "a live incoming hop still mints, an exact path grant still mints, and a tighter path grant still mints — the deny did not occupy the pair",
      legalId,
    ),
    expect(released && rt.hires.size === 1, 4, "that funded work still releases after the plate refuse", hireId),
