...
import { ASHLAR_TLDR, analog } from "./story.js";
import { fundHire, mustDispatch, offerHire } from "./hire-flow.js";
import type { TapResult } from "./sprint-procurement.js";

export interface RateFreshScenario {
  id: string;
  clockStart: string;
  genesisNonce: string;
...
export interface RateFreshReport {
  ok: boolean;
  results: TapResult[];
...
export function loadRateFresh(path: string): RateFreshScenario {
  return JSON.parse(readFileSync(path, "utf8")) as RateFreshScenario;
}
...
export function runRateFresh(scenario: RateFreshScenario): RateFreshReport {
  ...
  const live = offerHire(rt, {
    ...
    seller: vendor.id,
    sku: "research.brief",
    spec: "an empty pit does not waive the band",
    price,
    intentId,
  });
  ...
  const afterSneak = {
    denied: deniedRule(sneak, "market.rate_fresh"),
    flipDenied: deniedRule(flip, "market.rate_fresh"),
    bandAllows: allowedRule(sneak, "mm.spread_bound"),
    partyAllows: allowedRule(sneak, "market.fx_party"),
  ...
  const afterLegal = {
    rateAllows: allowedRule(legalAttempt, "market.rate_fresh"),
    openAllows: allowedRule(openAttempt, "market.rate_fresh"),
    quotes: rt.quotes.size,
  };
  ...
  must(rt.dispatch(cmd("hire.deliver", vendor.id, { hireId, deliverable: { n: 1 } })), "deliver after ashlar deny");
  must(rt.dispatch(cmd("envelope.require", vendor.id, { hireId })), "require");
  must(
    rt.dispatch(cmd("envelope.submit", desk.id, { hireId, nonce: `nonce-${hireId}` })),
    "submit after ashlar deny",
  );
  ...
  const results: TapResult[] = [
    expect(
      hired.replayed !== true &&
        ...
        quotesBeforeSneak === 1,
      1,
      "a listed seller still funds a hire, and an open FX room still opens with nobody on the pit, with no off-band conversion written yet",
      rfqId,
    ),
    expect(
      afterSneak.denied &&
        ...
        afterSneak.firstDeny === "market.rate_fresh" &&
        afterSneak.flipFirst === "market.rate_fresh" &&
        afterSneak.quotes === quotesBeforeSneak &&
        afterSneak.funded === "funded" &&
        afterSneak.noMaker,
      2,
      "minting an FX window at half price with no maker is market.rate_fresh — not a 200bps miss by the maker, not a vendor conversion while a maker sits, not a conversion that pays nothing",
    ),
    expect(
      legal.replayed !== true &&
        open.replayed !== true &&
        afterLegal.rateAllows &&
        afterLegal.openAllows &&
        afterLegal.quotes === quotesBeforeSneak + 2 &&
        legalId.startsWith("qte_") &&
        openId.startsWith("qte_"),
      3,
      "the market maker still mints an in-band window after sitting, and a par window still mints — the deny did not occupy the room",
      legalId,
    ),
    expect(
      ...
      4,
      "the maker's window still converts — an empty pit does not waive the band",
      String(payout),
    ),
    expect(released && rt.hires.size === 1, 5, "that funded work still releases after the ashlar refuse", hireId),
