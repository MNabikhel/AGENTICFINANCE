...
export const FILM_TLDR =
  "A founder funded an $800 hire. Settling the empty USDC clearing book was clearing.settle_state — a system speaker is not this deny, a junior speaker is not this deny, the funded USD book is not this deny. No window minted. The USD book with an open leg still settled, and settling it again right after was the same refuse. That funded work still released. An empty book is not a settlement photo.";
...
export function autoBeat(input: {
  ...
  if (cmd.type === "mandate.issue_intent") {
    ...
    if (decision.verdict === "deny") {
      ...
      if (rule?.ruleId === "mandate.root_party") {
        return {
          seq: input.seq,
          at: input.at,
          headline: `${who} cannot mint a root in someone else's name`,
          body: "Someone else's name is not a root slip to mint. Mint a root in your own name, or ask a human or treasury. A missing subject is a different object. A nested child is a different object. A junior mint is a different object. A vendor verb is a different object.",
          tone: "deny",
          commandType: cmd.type,
        };
      }
