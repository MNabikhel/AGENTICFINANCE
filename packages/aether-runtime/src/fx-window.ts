...
import { PANE_TLDR, analog } from "./story.js";
import { fundHire, mustDispatch, offerHire } from "./hire-flow.js";
import type { TapResult } from "./sprint-procurement.js";

export interface FxWindowScenario {
  id: string;
  clockStart: string;
  genesisNonce: string;
...
export interface FxWindowReport {
  ok: boolean;
  results: TapResult[];
...
export function loadFxWindow(path: string): FxWindowScenario {
  return JSON.parse(readFileSync(path, "utf8")) as FxWindowScenario;
}
...
export function runFxWindow(scenario: FxWindowScenario): FxWindowReport {
  ...
  const live = offerHire(rt, {
    ...
    seller: vendor.id,
    sku: "research.brief",
    spec: "an FX SKU is a window, not a good",
    price: hirePrice,
    intentId,
  });
  ...
  const afterSneak = {
    denied: deniedRule(sneak, "market.fx_window"),
  ...
  const settled = must(rt.dispatch(cmd("market.fx_settle", vendor.id, { quoteId })), "settle after pane deny");
  const payout = (settled.data as { payout: number }).payout;

  must(rt.dispatch(cmd("hire.deliver", vendor.id, { hireId, deliverable: { n: 1 } })), "deliver after pane deny");
  must(rt.dispatch(cmd("envelope.require", vendor.id, { hireId })), "require");
  must(
    rt.dispatch(cmd("envelope.submit", desk.id, { hireId, nonce: `nonce-${hireId}` })),
    "submit after pane deny",
  );
  ...
  const results: TapResult[] = [
    ...
    expect(
      afterSneak.denied &&
        ...
        afterSneak.firstDeny === "market.fx_window" &&
        afterSneak.quotes === quotesBeforeSneak,
      2,
      "an FX SKU with no window is market.fx_window — not a missing SKU, not a swapped pair, not a corpse mint",
      rfqId,
    ),
    expect(
    ...
    expect(released && rt.hires.size === 1, 4, "that funded work still releases after the pane refuse", hireId),
