...
import { CORBEL_TLDR, analog } from "./story.js";
import { fundHire, mustDispatch, offerHire } from "./hire-flow.js";
import type { TapResult } from "./sprint-procurement.js";

export interface NestPartyScenario {
  id: string;
  clockStart: string;
  genesisNonce: string;
...
export interface NestPartyReport {
  ok: boolean;
  results: TapResult[];
...
export function loadNestParty(path: string): NestPartyScenario {
  return JSON.parse(readFileSync(path, "utf8")) as NestPartyScenario;
}
...
export function runNestParty(scenario: NestPartyScenario): NestPartyReport {
  ...
  const live = offerHire(rt, {
    ...
    seller: vendor.id,
    sku: "research.brief",
    spec: "a nested hop under another principal is not a nested handshake",
    price,
    intentId,
  });
  ...
  const afterSneak = {
    denied: deniedRule(sneak, "kya.nest_party"),
    floorDenied: deniedRule(floor, "kya.nest_party"),
    ...
    studAllows: allowedRule(sneak, "kya.path_tighter"),
    plateAllows: allowedRule(sneak, "kya.path_live"),
    ...
    hopAHops: liveHops(rt, hopA.id, scout.id).length,
  ...
  const afterLegal = {
    nestAllows: allowedRule(legalAttempt, "kya.nest_party"),
    tighterAllows: allowedRule(tighterAttempt, "kya.nest_party"),
    joistAllows: allowedRule(legalAttempt, "kya.nest_tighter"),
    tighterJoistAllows: allowedRule(tighterAttempt, "kya.nest_tighter"),
    scoutHops: liveHops(rt, founder.id, scout.id).length,
    vendorHops: liveHops(rt, founder.id, vendor.id).length,
    size: rt.kya.attestations.size,
  };

  must(rt.dispatch(cmd("hire.deliver", vendor.id, { hireId, deliverable: { n: 1 } })), "deliver after corbel deny");
  must(rt.dispatch(cmd("envelope.require", vendor.id, { hireId })), "require");
  must(
    rt.dispatch(cmd("envelope.submit", desk.id, { hireId, nonce: `nonce-${hireId}` })),
    "submit after corbel deny",
  );
  ...
  const results: TapResult[] = [
    expect(
      hired.replayed !== true &&
        ...
        sizeBeforeSneak === 1,
      1,
      "a listed seller still funds a hire, and a parent hop still mints, with no cross-principal nested grant written yet",
      parentId,
    ),
    expect(
      afterSneak.denied &&
        ...
        afterSneak.studAllows &&
        afterSneak.plateAllows &&
        afterSneak.childAllows &&
        afterSneak.firstDeny === "kya.nest_party" &&
        afterSneak.floorFirst === "kya.nest_party" &&
        afterSneak.hops === hopsBeforeSneak &&
        afterSneak.hopAHops === 0 &&
        afterSneak.size === sizeBeforeSneak &&
        afterSneak.funded === "funded",
      2,
      "minting a nested hop under another principal is kya.nest_party — not a nested grant wider than its parent, not a dead parent hop, not whose name a handshake is in",
    ),
    expect(
      legal.replayed !== true &&
        ...
        afterLegal.joistAllows &&
        afterLegal.tighterJoistAllows &&
      ...
      3,
      "an exact same-principal nested grant still mints, and a tighter same-principal nested grant still mints — the deny did not occupy the pair",
      legalId,
    ),
    expect(released && rt.hires.size === 1, 4, "that funded work still releases after the corbel refuse", hireId),
