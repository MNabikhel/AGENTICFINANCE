...
export interface FxQuoteReport {
  ok: boolean;
  results: TapResult[];
...
export function loadFxQuote(path: string): FxQuoteScenario {
  return JSON.parse(readFileSync(path, "utf8")) as FxQuoteScenario;
}
...
export function runFxQuote(scenario: FxQuoteScenario): FxQuoteReport {
  ...
  const paper = scenario.quotes.paper!;
  ...
  const live = offerHire(rt, {
    ...
    seller: vendor.id,
    sku: "research.brief",
    spec: "a research quote is not a conversion window",
    price: hirePrice,
    intentId,
  });
  ...
  const paperQuoteId = papered.quoteId;
  const consumedBeforeSneak = rt.consumedQuotes.has(paperQuoteId);

  const sneak = rt.dispatch(cmd("market.fx_settle", vendor.id, { quoteId: paperQuoteId }));
  const afterSneak = {
    denied: deniedRule(sneak, "market.fx_quote"),
    ...
    hireAllows: allowedRule(sneak, "hire.not_fx"),
    firstDeny: sneak.ok ? undefined : sneak.error.decision?.remediation?.ruleId,
    consumed: rt.consumedQuotes.has(paperQuoteId),
  };
  ...
  const settled = must(rt.dispatch(cmd("market.fx_settle", vendor.id, { quoteId })), "settle after paper deny");
  const payout = (settled.data as { payout: number }).payout;

  must(rt.dispatch(cmd("hire.deliver", vendor.id, { hireId, deliverable: { n: 1 } })), "deliver after paper deny");
  must(rt.dispatch(cmd("envelope.require", vendor.id, { hireId })), "require");
  must(
    rt.dispatch(cmd("envelope.submit", desk.id, { hireId, nonce: `nonce-${hireId}` })),
    "submit after paper deny",
  );
  ...
  const results: TapResult[] = [
    ...
    expect(
      afterSneak.denied &&
        ...
        afterSneak.hireAllows &&
        afterSneak.firstDeny === "market.fx_quote" &&
        afterSneak.consumed === false &&
        consumedBeforeSneak === false,
      2,
      "a research quote is market.fx_quote — not a swapped pair, not a missing window, not a missing dest book",
      paperQuoteId,
    ),
    expect(
      quoted.replayed !== true &&
        rt.consumedQuotes.has(quoteId) &&
        !rt.consumedQuotes.has(paperQuoteId) &&
        payout === expectedPayout &&
        rt.ledger.balanceByName("research-vendor:usdc").amount === expectedPayout,
      3,
      "a real window still converts — the deny did not consume the research quote",
      quoteId,
    ),
    expect(released && rt.hires.size === 1, 4, "that funded work still releases after the paper refuse", hireId),
