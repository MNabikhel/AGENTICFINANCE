...
import { YEAR_TLDR, analog } from "./story.js";
import { fundHire, mustDispatch, offerHire } from "./hire-flow.js";
import type { TapResult } from "./sprint-procurement.js";

export interface KyaWindowScenario {
  id: string;
  clockStart: string;
  genesisNonce: string;
...
export interface KyaWindowReport {
  ok: boolean;
  results: TapResult[];
...
export function loadKyaWindow(path: string): KyaWindowScenario {
  return JSON.parse(readFileSync(path, "utf8")) as KyaWindowScenario;
}
...
export function runKyaWindow(scenario: KyaWindowScenario): KyaWindowReport {
  ...
  const live = offerHire(rt, {
    ...
    seller: vendor.id,
    sku: "research.brief",
    spec: "under a one-year handshake",
    price,
    intentId,
  });
  const hired = must(live.attempt, "hire under one-year hop");
  ...
  const sneak = rt.dispatch(
    cmd("kya.attest", founder.id, {
      ...
      expiresAt: scenario.farExpiresAt,
    }),
  );
  const afterSneak = {
    denied: deniedRule(sneak, "kya.mint_window"),
  ...
  const reached = must(
    rt.dispatch(
      cmd("kya.attest", founder.id, { delegateId: scout.id, maxAutonomy: scenario.grant.maxAutonomy }),
    ),
    "one-year scout hop",
  );
  const scoutHopId = (reached.data as { id: string }).id;

  must(rt.dispatch(cmd("hire.deliver", vendor.id, { hireId, deliverable: { n: 1 } })), "deliver after year deny");
  must(rt.dispatch(cmd("envelope.require", vendor.id, { hireId })), "require");
  must(
    rt.dispatch(cmd("envelope.submit", desk.id, { hireId, nonce: `nonce-${hireId}` })),
    "submit after year deny",
  );
  ...
  const results: TapResult[] = [
    expect(
      ...
      1,
      "desk hire.create allows and funds under a one-year handshake",
      hireId,
    ),
    expect(
      afterSneak.denied &&
        ...
        afterSneak.firstDeny === "kya.mint_window" &&
        afterSneak.hops === hopsBeforeSneak,
      2,
      "a hop that outlives one year is kya.mint_window — not standing identity",
    ),
    expect(
      reached.replayed !== true && liveHops(rt, founder.id, scout.id).length === 1,
      3,
      "a one-year hop still mints — the deny did not occupy the pair",
      scoutHopId,
    ),
    expect(released && rt.hires.size === 1, 4, "that funded work still releases after the century refuse", hireId),
