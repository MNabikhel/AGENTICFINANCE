...
import { PAUSE_TLDR, analog } from "./story.js";
import { fundHire, mustDispatch, offerHire } from "./hire-flow.js";
import type { TapResult } from "./sprint-procurement.js";

export interface ApprovalPendingScenario {
  id: string;
  clockStart: string;
  genesisNonce: string;
...
export function loadApprovalPending(path: string): ApprovalPendingScenario {
  return JSON.parse(readFileSync(path, "utf8")) as ApprovalPendingScenario;
}
...
export function runApprovalPending(scenario: ApprovalPendingScenario): ApprovalPendingReport {
  ...
  if (ticket) rt.clock.set(ticket.expiresAt);
  ...
  const afterSneak = {
    denied: deniedRule(sneak, "approval.pending"),
    ...
    expired: store?.status === "expired",
  };

  must(rt.dispatch(cmd("hire.deliver", vendor.id, { hireId, deliverable: { n: 1 } })), "deliver after pause deny");
  must(rt.dispatch(cmd("envelope.require", vendor.id, { hireId })), "require");
  must(
    rt.dispatch(cmd("envelope.submit", desk.id, { hireId, nonce: `nonce-${hireId}` })),
    "submit after pause deny",
  );
  ...
  const results: TapResult[] = [
    ...
    expect(
      escalatedRule(paused.attempt, "approval.threshold") &&
        ...
        afterSneak.firstDeny === "approval.pending" &&
        afterSneak.hires === hiresBeforePause &&
        afterSneak.consumed === false &&
        afterSneak.reserved === false &&
        afterSneak.expired,
      2,
      "yes after the pause dies is approval.pending — not a missing ticket, not a stale command; the quote is free again",
      paused.quoteId,
    ),
    expect(released && rt.hires.size === 1, 3, "that funded work still releases after the pause refuse", hireId),
