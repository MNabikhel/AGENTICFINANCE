...
import { SKU_TLDR, analog } from "./story.js";
import { fundHire, mustDispatch, offerHire } from "./hire-flow.js";
import type { TapResult } from "./sprint-procurement.js";

export interface PaymentSkusScenario {
  id: string;
  clockStart: string;
  genesisNonce: string;
...
export interface PaymentSkusReport {
  ok: boolean;
  results: TapResult[];
...
export function loadPaymentSkus(path: string): PaymentSkusScenario {
  return JSON.parse(readFileSync(path, "utf8")) as PaymentSkusScenario;
}
...
export function runPaymentSkus(scenario: PaymentSkusScenario): PaymentSkusReport {
  ...
  const live = offerHire(rt, {
    ...
    seller: vendor.id,
    sku: "research.brief",
    spec: "listed sku still hires",
    price: firstPrice,
    intentId,
  });
  const hired = must(live.attempt, "hire listed sku");
  ...
  const sneak = offerHire(rt, {
    buyer: desk.id,
    seller: vendor.id,
    sku: scenario.sneakSku,
    spec: "catalog good not on the list",
    price: sneakPrice,
    intentId,
  });
  const afterSneak = {
    denied: deniedRule(sneak.attempt, "payment.allowed_skus"),
    knownAllows: allowedRule(sneak.attempt, "market.known_sku"),
    ...
    fxAllows: allowedRule(sneak.attempt, "hire.not_fx"),
  ...
  must(rt.dispatch(cmd("hire.deliver", vendor.id, { hireId, deliverable: { n: 1 } })), "deliver after sku deny");
  must(rt.dispatch(cmd("envelope.require", vendor.id, { hireId })), "require");
  must(
    rt.dispatch(cmd("envelope.submit", desk.id, { hireId, nonce: `nonce-${hireId}` })),
    "submit after sku deny",
  );
  ...
  const results: TapResult[] = [
    ...
    expect(
      afterSneak.denied &&
        ...
        afterSneak.fxAllows &&
        afterSneak.firstDeny === "payment.allowed_skus" &&
        afterSneak.hires === hiresBeforeSneak &&
        afterSneak.consumed === false &&
        afterSneak.quotes === quotesBeforeSneak + 1,
      2,
      "unlisted catalog good is payment.allowed_skus — not a missing SKU, not a listed payee",
      sneak.quoteId,
    ),
