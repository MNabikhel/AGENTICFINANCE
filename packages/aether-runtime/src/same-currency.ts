...
import { MIX_TLDR, analog } from "./story.js";
import { fundHire, mustDispatch, offerHire } from "./hire-flow.js";
import type { TapResult } from "./sprint-procurement.js";

export interface SameCurrencyScenario {
  id: string;
  clockStart: string;
  genesisNonce: string;
  ...
  sneak: { amount: number; currency: "USD_SIM" };
...
export interface SameCurrencyReport {
  ok: boolean;
  results: TapResult[];
...
export function loadSameCurrency(path: string): SameCurrencyScenario {
  return JSON.parse(readFileSync(path, "utf8")) as SameCurrencyScenario;
}
...
export function runSameCurrency(scenario: SameCurrencyScenario): SameCurrencyReport {
  ...
  const live = offerHire(rt, {
    ...
    seller: vendor.id,
    sku: "research.brief",
    spec: "a mixed journal is not a conversion",
    price: hirePrice,
    intentId,
  });
  ...
  const destBefore = rt.ledger.balanceByName("research-vendor:usdc").amount;
  const journalsBefore = rt.journals.length;

  const sneak = rt.dispatch(
    cmd("ledger.transfer", treasury.id, {
      fromAccount: "treasury:cash",
      toAccount: "research-vendor:usdc",
      amount: scenario.sneak,
    }),
  );
  const afterSneak = {
    denied: deniedRule(sneak, "ledger.same_currency"),
    ...
    safeAllows: allowedRule(sneak, "ledger.safe_balance"),
    ...
    dest: rt.ledger.balanceByName("research-vendor:usdc").amount,
  ...
  const settled = must(rt.dispatch(cmd("market.fx_settle", vendor.id, { quoteId })), "settle after mix deny");
  ...
  must(rt.dispatch(cmd("hire.deliver", vendor.id, { hireId, deliverable: { n: 1 } })), "deliver after mix deny");
  must(rt.dispatch(cmd("envelope.require", vendor.id, { hireId })), "require");
  must(
    rt.dispatch(cmd("envelope.submit", desk.id, { hireId, nonce: `nonce-${hireId}` })),
    "submit after mix deny",
  );
  ...
  const results: TapResult[] = [
    ...
    expect(
      afterSneak.denied &&
        ...
        afterSneak.safeAllows &&
        afterSneak.roleAllows &&
        afterSneak.firstDeny === "ledger.same_currency" &&
        afterSneak.treasury === treasuryBefore &&
        afterSneak.dest === destBefore &&
        afterSneak.journals === journalsBefore,
      2,
      "USD into a USDC book is ledger.same_currency — not a missing book, not empty cash, not a mint",
      "treasury:cash → research-vendor:usdc",
    ),
    expect(
      quoted.replayed !== true &&
        rt.consumedQuotes.has(quoteId) &&
        payout === expectedPayout &&
        vendorUsdcAfter === destBefore + expectedPayout,
      3,
      "a real window still converts — the deny did not post a mixed journal",
      quoteId,
    ),
    expect(released && rt.hires.size === 1, 4, "that funded work still releases after the mix refuse", hireId),
