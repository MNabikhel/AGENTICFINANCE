...
import { HATCH_TLDR, analog } from "./story.js";
import { fundHire, mustDispatch, offerHire } from "./hire-flow.js";
import type { TapResult } from "./sprint-procurement.js";

export interface HatchFreshScenario {
  id: string;
  clockStart: string;
  genesisNonce: string;
...
export interface HatchFreshReport {
  ok: boolean;
  results: TapResult[];
...
export function loadHatchFresh(path: string): HatchFreshScenario {
  return JSON.parse(readFileSync(path, "utf8")) as HatchFreshScenario;
}
...
export function runHatchFresh(scenario: HatchFreshScenario): HatchFreshReport {
  ...
  const live = offerHire(rt, {
    ...
    seller: vendor.id,
    sku: "research.brief",
    spec: "a closed hatch is not a range",
    price,
    intentId,
  });
  ...
  const sneak = rt.dispatch(hatchBody(founder.id, desk.id, payees, scenario.sneak.task, undefined, 0));
  const zero = rt.dispatch(hatchBody(founder.id, desk.id, payees, "a zero lid is not a range", 0, 0));
  const afterSneak = {
    denied: deniedRule(sneak, "mandate.lid_fresh"),
    zeroDenied: deniedRule(zero, "mandate.lid_fresh"),
  ...
  const legalAttempt = rt.dispatch(hatchBody(founder.id, desk.id, payees, scenario.legal.task, 80_000, 80_000));
  const legal = must(legalAttempt, "exact live band");
  const legalId = (legal.data as { payload: { id: MandateId } }).payload.id;
  const openAttempt = rt.dispatch(hatchBody(founder.id, desk.id, payees, scenario.open.task, undefined, 500_000));
  const open = must(openAttempt, "open floor");
  const openId = (open.data as { payload: { id: MandateId } }).payload.id;
  const afterLegal = {
    hatchAllows: allowedRule(legalAttempt, "mandate.lid_fresh"),
    openAllows: allowedRule(openAttempt, "mandate.lid_fresh"),
    size: rt.intents.size,
  };

  must(rt.dispatch(cmd("hire.deliver", vendor.id, { hireId, deliverable: { n: 1 } })), "deliver after hatch deny");
  must(rt.dispatch(cmd("envelope.require", vendor.id, { hireId })), "require");
  must(
    rt.dispatch(cmd("envelope.submit", desk.id, { hireId, nonce: `nonce-${hireId}` })),
    "submit after hatch deny",
  );
  ...
  const results: TapResult[] = [
    expect(
      ...
      1,
      "a listed seller still funds a hire with no closed hatch written yet",
      hireId,
    ),
    expect(
      afterSneak.denied &&
        ...
        afterSneak.firstDeny === "mandate.lid_fresh" &&
        afterSneak.zeroFirst === "mandate.lid_fresh" &&
        afterSneak.size === sizeBeforeSneak &&
        afterSneak.funded === "funded",
      2,
      "minting an amount_range whose max cannot admit a positive hire is mandate.lid_fresh — not a vacant slot, not a floor above the lid, not hire-time max",
    ),
    expect(
      legal.replayed !== true &&
        open.replayed !== true &&
        afterLegal.hatchAllows &&
        afterLegal.openAllows &&
        afterLegal.size === sizeBeforeSneak + 2 &&
        legalId.startsWith("mid_") &&
        openId.startsWith("mid_"),
      3,
      "a live lid still mints, and an open floor still mints — the deny did not write a corpse",
      legalId,
    ),
    expect(released && rt.hires.size === 1, 4, "that funded work still releases after the hatch refuse", hireId),
