...
export interface PolicyContext {
  ...
  hireNotFx?: boolean;              // false when hire.create would treat an FX window as a hireable good
  marketFresh?: boolean;
  ...
  kyaPartyOk?: boolean;             // false when attest/revoke names a principal that is not the actor (human/treasury exempt)
  aliasFree?: boolean;              // false when identity.register would reuse a runtime alias or its cash book
...
| 58 | `kya.party` | kya.attest / kya.revoke | actor is not the named principal, and is not a human or treasury | — | principal, or kill-switch role |
| 59 | `identity.unique_key` | identity.register | runtime alias already taken, or cash book already open (a second market maker collides on `market_maker:cash_usd`) | — | free alias and cash book |
...
| 64 | `mandate.unique_payment` | issue_payment | cart already has a payment mandate (same cart hash) | — | cart has no payment yet |
...
| 67 | `hire.not_fx` | hire.create | quote carries an FX window (`quote.fx`); windows settle, they are not hires | — | quote is not an FX window |
...
| `hire.test.ts` | deliver before funded denied; self-deal denied; illegal arrows and payment-required before deliver are `hire.state`; funding without cash is `ledger.sufficient`; funding a USDC hire from USD cash is `ledger.same_currency` |
