...
| 100 | `mandate.currency_fresh` | issue_intent with amount_range and payment.budget | lid and coffer name different currencies | — | same currency on both, or only one constraint present. Matching USD still mints. Matching USDC still mints. Vacant cap stays `mandate.occurrence_fresh`. A closed coffer stays `mandate.budget_fresh`. A floor above the lid stays `mandate.range_fresh`. A closed hatch is `mandate.lid_fresh`. Ink TAP is cart vs hire (`payment.currency_match`). Mix TAP is a mixed journal. Clash TAP is a mixed envelope at mint. |
