...
| 48 | `hire.party` | accept / deliver / envelope.require / refund / release / void | actor is not the seller (accept/deliver/require) or not the buyer/treasury (refund/release) or not the buyer/seller/treasury (void) | — | right party. Void TAP is tearing up funded escrow (`hire.state`). Party TAP is a stranger on a live hire. |
...
| 94 | `market.rfq_party` | market.close | actor is not the named buyer, and is not a human or treasury | — | own room, or a kill-switch role. Ghost shut stays `market.known_rfq`. Expired/shut stays `market.not_expired`. Shut TAP is a second desk closing the research desk's live RFQ. Completing funded work is legal. Closing the room does not kill an already-minted FX window. The store stays raw. Inspect labels `closed`. Closed wins over expired. Fold TAP is someone else's bid (`market.party`). |
