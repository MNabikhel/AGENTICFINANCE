...
A transfer from equity is `ledger.operating_book` — not a mint. Operating cash still funds an $800 hire. A transfer out of that escrow is `ledger.operating_book` — not an allocation. That funded work still releases. Overdraft stays `ledger.sufficient`. Still 87 rules.
...
An $800 hire empties the desk. A $400 second `hire.fund` is `ledger.sufficient` — same currency, operating cash, and the hire arrow still allow — and locks no escrow. That funded work still releases. Mint TAP is a transfer from equity. Stock TAP is empty MM USDC. Still 87 rules.
