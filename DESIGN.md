...
The three TAP demos exist. Further work is sequenced in [`docs/FOUNDATION.md`](docs/FOUNDATION.md): HTTP as a complete command bus, an OpenAPI document that is the bus, a clearing-window TAP, then adapter-shaped KYA issuers. Do not add a UI. Do not add a live rail. Do not grind inspect overlays unless the pin would otherwise lie.
