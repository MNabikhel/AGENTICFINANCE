...
export interface PolicyContext {
  ...
  fundsOk?: boolean;                // false when a transfer would overdraw the source book
}
```
...
| 56 | `ledger.sufficient` | ledger.transfer | source book balance < amount | — | source covers the cents (zero is legal) |
...
| `hire.test.ts` | deliver before funded denied; self-deal denied; illegal arrows and payment-required before deliver are `hire.state` |
