...
| 101 | `mandate.lid_fresh` | issue_intent with amount_range | max ≤ 0 | — | lid can still admit a positive hire. A live lid still mints. An open floor still mints. Vacant cap stays `mandate.occurrence_fresh`. A floor above the lid stays `mandate.range_fresh`. Lid TAP is hire-time max (`payment.amount_range`). Hatch TAP is a closed hatch. A closed coffer is `mandate.budget_fresh`. A mixed envelope is `mandate.currency_fresh`. |
