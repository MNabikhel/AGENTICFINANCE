...
An $800 hire funds under the auto-approve line. A $6,400 hire pauses as `approval.threshold`. After that ticket dies, `approval.resolve` is `approval.pending` — the ticket still exists; a stale command is not this deny — and writes no hire. The quote is free again. That funded work still releases. Velocity TAP is a hot hour. Deny TAP is a cached no. Replay TAP is a retry of an allow. Docket TAP is a missing ticket. Still 87 rules.
...
An $800 hire funds under the auto-approve line. `approval.resolve` on a ghost ticket is `approval.known` — a dead pause still allows; a stale command is not this deny — and writes no ticket. That funded work still releases. Pause TAP is a dead ticket. Replay TAP is a retry of an allow. Velocity TAP is a hot hour. Still 87 rules.
