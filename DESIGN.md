...
| 105 | `kya.path_tighter` | kya.attest naming a principal other than the speaker, when the speaker has a live path from that principal | child maxAutonomy (omit = L5) is above the leaf incoming hop's maxAutonomy | — | path grant is at or below the incoming hop. Exact match still mints. A tighter child still mints. Speaker granting in their own name is not this deny. No live path does not set the flag. A nested grant wider than its parent stays `kya.nest_tighter`. A grant below the desk stays `kya.grant_fresh`. An agent over-grant stays `kya.capability_subset`. Well TAP equal hops still mint. Stud TAP is a grant wider than the incoming hop at mint. |
