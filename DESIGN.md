...
| `aether-mandate` | `issueIntent`, `issueCart`, `issuePayment`, `revokeIntent`, `revokeCart`, `revokePayment`, `verifyChain` | mandate table |
...
| `aether-market` | `createRfq`, `submitQuote`, `acceptQuote`, `withdrawQuote`, `closeRfq` | rfq/quote tables |
...
| `mandate.revoke_cart` | `POST /v1/carts/{id}/dump` | `aether_mandate_revoke_cart` |
| `mandate.revoke_payment` | `POST /v1/checks/{id}/spike` | `aether_mandate_revoke_payment` |
...
| `market.close` | `POST /v1/rfqs/{id}/close` | `aether_market_close` |
...
export type MandateConstraint =
  | {
      ...
      currency: CurrencyCode;
      min?: number;           // omit is an open floor. min === max still mints. min > max is mandate.range_fresh
      max: number;            // max ≤ 0 is mandate.lid_fresh. Hire-time overflow is payment.amount_range
    }
  | {
      type: "payment.budget";
      currency: CurrencyCode;
      max: number;            // remaining computed at eval. max ≤ 0, or max below an amount_range floor, is mandate.budget_fresh. Different currency than amount_range is mandate.currency_fresh
    }
  | {
  ...
  | {
      ...
      max: AutonomyLevel;     // mint below subject's live rung is mandate.cap_fresh. Hire-time climb is ladder.max_autonomy_constraint
    };
...
export interface PolicyContext {
  ...
  revokeStateOk?: boolean;          // false when kya.revoke would tombstone nothing and block nothing new — a tombstone is not a second tombstone
  settleWindowOk?: boolean;         // false when clearing.settle_window names a currency whose open book has zero legs — an empty book is not a settlement photo
  ...
  cadenceReachOk?: boolean;         // false when issue_intent would write a recurrence whose next slot opens at or after the seven-day exp
  rangeMintOk?: boolean;            // false when issue_intent would write an amount_range whose min exceeds max
  budgetMintOk?: boolean;           // false when issue_intent would write a payment.budget that cannot admit an amount the lid would allow
  currencyMintOk?: boolean;         // false when issue_intent would write an amount_range and a payment.budget in different currencies
  lidMintOk?: boolean;              // false when issue_intent would write an amount_range whose max cannot admit a positive hire
  capMintOk?: boolean;              // false when issue_intent would write an aether.max_autonomy below the named subject's live rung
  grantMintOk?: boolean;            // false when kya.attest would write a maxAutonomy below the named delegate's live rung
...
| 05 | `mandate.subject_is_actor` | settle / hosted host.subscribe | `intent.subjectId !== actor.id` | — | match. Subject TAP is a second desk's `hire.fund`. Seat TAP is a hosted subscribe row. Wearing someone else's unused slip at `hire.create` stays `hire.slip_party`. |
| 06 | `payment.currency_match` | settle | cart/payment/amount currencies differ | — | match. Ink TAP is a USDC cart on a USD hire. Clash TAP is mixed lid/coffer at mint (`mandate.currency_fresh`). |
| 07 | `payment.amount_range` | settle if constraint present | amount outside `[min,max]` or currency mismatch. Minting min > max is `mandate.range_fresh`. Minting max ≤ 0 is `mandate.lid_fresh`. | — | in range. Lid TAP is hire-time max. Gulf TAP is a corpse band at mint. Hatch TAP is a closed lid at mint. |
| 08 | `payment.budget` | settle if constraint present | `spentAgainstIntent + amount > max`. Minting max ≤ 0, or max below an amount_range floor, is `mandate.budget_fresh`. Minting a different currency than the lid is `mandate.currency_fresh`. | — | remaining ≥ amount. Purse TAP is hire-time envelope. Coffer TAP is a closed envelope at mint. Clash TAP is a mixed envelope at mint. |
...
| 14 | `ladder.max_autonomy_constraint` | new spend if `aether.max_autonomy` present | actor.level > max (over-autonomy abuse) | — | actor.level ≤ max. Completing a funded hire after a climb is legal. Minting a cap below the named subject's live rung is `mandate.cap_fresh`. Ceiling TAP is a climb after mint. Eave TAP is a cap below the desk at mint. |
...
| 23 | `mm.spread_bound` | MM quote | `rateE6` outside `[980_000, 1_020_000]` (200 bps) | — | inside band. A vendor off-band with no maker is `market.rate_fresh`. |
...
| 33 | `mandate.child_tighter` | `mandate.issue_intent` with `parentId` | child amount_range max wider or floor lower than parent / budget / SKUs / payees / instruments / max autonomy wider than parent | — | child is a subset |
...
| 36 | `market.not_expired` | quote / hire.create / fx_settle | RFQ, quote, or FX `validUntil` ≤ now | — | in window. A shut room plus a foreign hire stays this first. Poach TAP is hiring from a live room (`hire.room_party`). |
...
| 39 | `market.known_rfq` | quote / hire.create | RFQ (or the quote’s RFQ) does not exist | — | room exists. Hall TAP is a ghost RFQ. Room TAP is a closed guest list. Shelf TAP is a ghost SKU. Guest TAP is a missing invitee. Writ TAP is a ghost slip. A ghost quote plus a foreign hire stays this first. Poach TAP is hiring from a live room (`hire.room_party`). |
...
| 41 | `hire.quote_unspent` | hire.create | quote already produced a hire, an FX settle, or is reserved by an open approval | — | quote unused. A spent quote plus a foreign hire stays this first. Someone else's first hire is `hire.room_party`. |
...
| 43 | `mandate.known_intent` | hire.create / issue_cart | intentId not in this world | — | intent exists. Writ TAP is a ghost slip. Heir TAP is a dead parent. Name TAP is whose name a handshake is in. A ghost slip plus a foreign hire stays this first. Guise TAP is wearing a live unused slip (`hire.slip_party`). |
...
| 45 | `mandate.known_payment` | mandate.revoke_payment | paymentId not in this world | — | payment exists. Spike TAP is a ghost check. Occupancy stays mandate.unique_payment. A dead cart at fund stays mandate.chain_integrity. Completing funded work is legal. |
| 46 | `approval.known` | approval.resolve | approvalId not in this world | — | ticket exists |
| 47 | `approval.pending` | approval.resolve | ticket expired or already resolved | — | ticket pending |
| 48 | `hire.party` | accept / deliver / envelope.require / refund / release / void | actor is not the seller (accept/deliver/require) or not the buyer/treasury (refund/release) or not the buyer/seller/treasury (void) | — | right party. Void TAP is tearing up funded escrow (`hire.state`). Party TAP is a stranger on a live hire. Poach TAP is hiring from someone else's room (`hire.room_party`). |
| 49 | `mandate.known_parent` | issue_intent with parentId | parentId not in this world | — | parent exists. Root TAP is a ghost parent. Heir TAP is a dead parent. Nest TAP is a dead hop. |
| 50 | `identity.known` | freeze / unfreeze / rotate / ladder.set / kya.attest / kya.revoke / issue_cart / issue_intent / market.rfq | named agentId / delegateId / principalId / merchantId / subjectId / invitedSellerIds not in this world | — | agent registered. Guest TAP is a missing invitee. Room TAP is a closed guest list. Lock TAP is someone else's key. |
| 51 | `hire.state` | accept / fund / deliver / refund / release / void / envelope.submit / envelope.require | command is not a legal arrow from the hire’s current state; payment-required is only after deliver; void is offered or accepted only | — | legal transition. Void TAP is tearing up funded escrow. Arrow TAP is release before deliver. Refund TAP is unwind funded escrow. Bare TAP is deliver before fund. |
| 52 | `ladder.legal` | ladder.set | skip a rung, omit a required gate, list `kill_switch_tested` without a freeze test, or wrong approver | — | legal climb (`any→L0` always). Rung TAP is skip L2→L4. Climb TAP is a handshake ceiling. Night Watch’s premature L5 is the freeze-test gate. |
| 53 | `kya.not_self` | kya.attest | grantor would attest themselves | — | handshake with another agent |
| 54 | `kya.known_parent` | kya.attest with parentId | parentId not in this world’s graph | — | parent hop exists (a dead hop is `kya.parent_fresh`) |
| 55 | `ledger.known_account` | ledger.transfer / named ledger.balances / market.fx_settle | fromAccount, toAccount, name, accountId, or the FX actor’s USDC book is not in this world | — | book exists |
| 56 | `ledger.same_currency` | ledger.transfer / hire.fund | the two books disagree, the stated amount currency disagrees, or fund would lock a cash book into an escrow of a different currency | — | one currency (`market.fx_settle` to convert). Mix TAP is USD into a USDC book. Wallet TAP is a missing dest book. Cash TAP is empty operating cash. Mint TAP is a transfer from equity. Priced TAP is catalog currency. Paper TAP is settling a research quote as FX. |
| 57 | `ledger.sufficient` | ledger.transfer / hire.fund / market.fx_settle | source book (transfer fromAccount, buyer cash at fund, or vendor USD at FX settle) balance < amount | — | source covers the cents (zero is legal) |
| 58 | `kya.known_attestation` | kya.revoke with attestationId | attestationId not in this world’s graph, or belongs to a different principal | — | handshake exists for this principal |
| 59 | `kya.party` | kya.attest / kya.revoke | actor is not the named principal, and is not a human or treasury (omitted principalId is the speaker) | — | principal, or kill-switch role |
| 60 | `identity.unique_key` | identity.register | runtime alias already taken, or an operating book already open (USD cash; USDC for data_vendor / market_maker; a second market maker collides on `market_maker:cash_usd`) | — | free alias and operating books |
| 61 | `receipt.known` | receipt.get | receiptId not in this world | — | receipt exists |
| 62 | `identity.freeze_state` | freeze / unfreeze | freeze of an already-frozen agent, or unfreeze of an agent that is not frozen | — | freeze delta matches live state |
| 63 | `kya.unique_live` | kya.attest | a live (non-revoked) hop already exists for this principal→delegate pair | — | no live hop for this pair |
| 64 | `hire.unique_cart` | issue_cart with hireId | live hire already has a cartId | — | hire has no cart yet. Someone else's first cart is `mandate.checkout_party`. Trolley TAP is filling. Dump TAP is tearing. |
| 65 | `mandate.unique_payment` | issue_payment | cart already has a payment mandate (same cart hash) that is not revoked | — | cart has no occupying payment yet. Inspect / snapshot label that cart `bound`, not `live`. Inspect / snapshot label that payment `live` (or `funded` after escrow moves, or `revoked` after spike). Cart TAP is occupancy. Dust TAP is a first payment on a stale unpaid cart. Spike TAP frees occupancy. Someone else's first payment is `mandate.checkout_party`. |
| 66 | `market.sku_currency` | quote / hire.create | listed SKU priced in a currency the catalog does not name for that SKU | — | price currency is listed |
| 67 | `market.fx_pair` | quote / fx_settle | FX window on a non-FX SKU, swapped from/to, or price not in `from` (this rail journals USD_SIM → USDC_SIM) | — | USD_SIM → USDC_SIM priced in from |
| 68 | `hire.not_fx` | hire.create | quote carries an FX window (`quote.fx`) or the SKU is an FX SKU; windows settle, they are not hires | — | quote is not an FX window or FX SKU |
| 69 | `approval.replay` | approval.resolve (approved) | paused command would not allow (stale quote, expired slip, missing pending command) | — | waived replay is still allow |
| 70 | `market.fx_window` | quote | listed FX SKU quoted without an `fx` window | — | FX SKU carries a window |
| 71 | `hire.bound_cart` | fund / release / envelope.submit | live hire has not bound a cart (and that cart’s payment); a body cartId is not a pointer | — | hire holds its cart and payment |
| 72 | `mm.known` | fx_settle (live FX quote) | no market_maker agent, or missing `market_maker:cash_usd` / `market_maker:cash_usdc` | — | MM and both books exist |
| 73 | `actor.known` | always when actorId is not system | actorId is not a registered agent (HTTP/MCP unknown alias is this string, not silent system) | — | speaker exists (or is system) |
| 74 | `ledger.safe_balance` | transfer / fund / refund / release / envelope.submit / fx_settle | posting the journal would leave a book outside `Number.isSafeInteger` (dest + amount, or the matching source/equity leg) | — | resulting books stay safe integers. Brim TAP is dest overflow. |
| 75 | `actor.system_scope` | always when actorId is system | command is not first-human bootstrap or a read (catalog / audit.query / audit.verify / balances / receipt.get / host.card) | — | system may bootstrap or read |
| 76 | `ledger.operating_book` | ledger.transfer (covered, same-currency) | from or to is equity or escrow (or any non-asset book) | — | both books are operating cash |
| 77 | `ladder.birth_rung` | identity.register | autonomyLevel is 5 | — | birth rung is L0–L4 (`ladder.set` 4→5 after a freeze test) |
| 78 | `kya.mint_fresh` | kya.attest | expiresAt ≤ now, or unparseable Instant (omit is one year from now) | — | handshake expires after now |
| 79 | `kya.mint_window` | kya.attest | expiresAt after now + one year (omit is that ceiling) | — | handshake expires within one year |
| 80 | `mandate.window_fresh` | issue_intent with execution_date | not_after already past, inverted window, or unparseable Instant (a future not_before still mints if it opens while the slip lives) | — | window can still contain a now |
| 81 | `mandate.window_reach` | issue_intent with execution_date | not_before at or after the slip's seven-day exp | — | window opens while the intent is live |
| 82 | `mandate.occurrence_fresh` | issue_intent with agent_recurrence | max_occurrences ≤ 0, or not a finite number (omit is unlimited and still mints when the gap fits) | — | cadence can still admit a first hire. Week TAP is a WEEKLY/MONTHLY that cannot admit a second hire (`mandate.cadence_reach`). |
| 83 | `mandate.parent_fresh` | issue_intent / hire.create / hire.accept / hire.fund with a parent | parent intent `exp` (unix seconds) is already past | — | parent still lives (completing a funded hire after that is legal) |
| 84 | `kya.parent_fresh` | kya.attest with a known parentId; hire.create / hire.fund / issue_intent along a nested hop | parent hop is expired or revoked (`hopStatus` is not live) | — | parent hop still lives (ghost stays `kya.known_parent`; a nested grant wider than its parent stays `kya.nest_tighter`; completing a funded hire after that is legal) |
| 85 | `market.fx_fresh` | quote with an `fx` object | `validUntil` ≤ now, or unparseable Instant | — | FX window still open at mint (settle of a later lapse stays `market.not_expired`). A conversion that pays nothing is `market.payout_fresh`. |
| 86 | `host.not_hosted` | host.subscribe | this instance is not a hosted operator (`Runtime.hosted` is false) | — | a hosted operator (`Runtime({ hosted: true })`). `host.card` is a read. Self-host is free. GitHub is not a checkout. |
| 87 | `host.human_authority` | hosted host.subscribe with a known intent | intent issuer is not `human_operator` or `treasury` | — | live intent issued by a human or treasury (ghost stays `mandate.known_intent`; expired stays `mandate.not_expired`; wrong subject stays `mandate.subject_is_actor`) |
| 88 | `host.unique_subscriber` | hosted host.subscribe with a known intent | this agent already has a subscription row | — | one subscriber, one row. Spend is not gated on the row. |
| 89 | `payment.allowed_payment_instruments` | hire.create / hire.fund if constraint present | stamped (or implied) instrument id not in the list | — | listed. This kernel stamps `sim-ledger`. Live-rail *type* stays `instrument.sim_only`. Rail TAP is a ghost id. Payee TAP is who. |
| 90 | `payment.reference` | hire.create / hire.fund if constraint present and a funded payment exists | cited `conditional_transaction_id` is not a funded payment's `transaction_id` | — | bind. Before any funded payment the constraint is still AP2-shaped catalog surface. Cite TAP is a ghost checkout hash. Completing funded work is legal. |
| 91 | `identity.party` | identity.rotate | actor is not the named agent, and is not a human or treasury (omit agentId is the speaker) | — | own key, or a kill-switch role. Ghost rotate stays `identity.known`. Frozen speaker stays `actor.not_frozen`. System stays `actor.system_scope`. Lock TAP is a vendor turning the desk's lock. Rotation is not a new identity. Retired keys still verify. |
| 92 | `market.party` | market.withdraw | actor is not the named seller, and is not a human or treasury | — | own bid, or a kill-switch role. Ghost fold stays `market.known_rfq`. Expired/folded stays `market.not_expired`. Spent stays `hire.quote_unspent`. Fold TAP is a second vendor pulling the research desk's live bid. A folded quote is not a hire. The store stays raw. Inspect labels `withdrawn`. |
| 93 | `mandate.party` | mandate.revoke | actor is not the named issuer, and is not a human or treasury | — | own unused slip, or a kill-switch role. Ghost rip stays `mandate.known_intent`. Expired/ripped stays `mandate.not_expired`. Rip TAP is a desk tearing the founder's unused slip. Completing funded work is legal. The store stays raw. Inspect labels `revoked`. Funded wins over revoked. |
| 94 | `market.rfq_party` | market.close | actor is not the named buyer, and is not a human or treasury | — | own room, or a kill-switch role. Ghost shut stays `market.known_rfq`. Expired/shut stays `market.not_expired`. Shut TAP is a second desk closing the research desk's live RFQ. Completing funded work is legal. Closing the room does not kill an already-minted FX window. The store stays raw. Inspect labels `closed`. Closed wins over expired. Fold TAP is someone else's bid (`market.party`). Poach TAP is hiring from someone else's room (`hire.room_party`). |
| 95 | `mandate.cart_party` | mandate.revoke_cart | actor is not the named merchant, hire buyer, or intent subject, and is not a human or treasury | — | own unused checkout, or a kill-switch role. Ghost dump stays `mandate.known_cart`. Expired/dumped/bound stays `mandate.not_expired`. Dump TAP is a second desk dumping the research desk's unused cart. Completing funded work is legal. Bound is when a payment occupies it. Unused dump frees hire occupancy. The store stays raw. Inspect labels `revoked`. Bound wins over revoked. Revoked wins over expired. Rip TAP is someone else's unused slip (`mandate.party`). Filling is `mandate.checkout_party`. |
| 96 | `mandate.payment_party` | mandate.revoke_payment | actor is not the named signer, payee, hire buyer, or intent subject, and is not a human or treasury | — | own unused check, or a kill-switch role. Ghost spike stays `mandate.known_payment`. Expired/spiked/funded stays `mandate.not_expired`. Spike TAP is a second desk spiking the research desk's unused payment. Completing funded work is legal. Funded is when escrow occupies it. Unused spike frees unique_payment occupancy. The store stays raw. Inspect labels `revoked`. Funded wins over revoked. Revoked wins over expired. Dump TAP is someone else's unused checkout (`mandate.cart_party`). Filling is `mandate.checkout_party`. |
| 97 | `mandate.cadence_reach` | issue_intent with agent_recurrence | frequency gap ≥ the seven-day slip life when the cap is omitted or greater than one (`WEEKLY` / `MONTHLY`) | — | next slot opens while the slip lives. Vacant cap stays `mandate.occurrence_fresh`. A one-shot WEEKLY still mints. DAILY still mints. Week TAP is a week on a seven-day slip. Hire still names `payment.recurrence`. A floor above the lid is `mandate.range_fresh`. |
| 98 | `mandate.range_fresh` | issue_intent with amount_range | min exceeds max | — | band can still admit an amount. Omit min is an open floor. min === max still mints. Vacant cap stays `mandate.occurrence_fresh`. A week that cannot admit a second hire stays `mandate.cadence_reach`. Lid TAP is hire-time max (`payment.amount_range`). Gulf TAP is a floor above the lid. A closed hatch is `mandate.lid_fresh`. A closed coffer is `mandate.budget_fresh`. |
| 99 | `mandate.budget_fresh` | issue_intent with payment.budget | max ≤ 0, or max below an amount_range floor | — | envelope can still admit an amount the lid would allow. A coffer that covers the floor still mints. An open floor still mints. Vacant cap stays `mandate.occurrence_fresh`. A floor above the lid stays `mandate.range_fresh`. Purse TAP is hire-time envelope (`payment.budget`). Coffer TAP is a closed coffer. A mixed envelope is `mandate.currency_fresh`. |
| 100 | `mandate.currency_fresh` | issue_intent with amount_range and payment.budget | lid and coffer name different currencies | — | same currency on both, or only one constraint present. Matching USD still mints. Matching USDC still mints. Vacant cap stays `mandate.occurrence_fresh`. A closed coffer stays `mandate.budget_fresh`. A floor above the lid stays `mandate.range_fresh`. A closed hatch is `mandate.lid_fresh`. Ink TAP is cart vs hire (`payment.currency_match`). Mix TAP is a mixed journal. Clash TAP is a mixed envelope at mint. Header TAP is a nested child in a different currency (`mandate.child_currency`). |
| 101 | `mandate.lid_fresh` | issue_intent with amount_range | max ≤ 0 | — | lid can still admit a positive hire. A live lid still mints. An open floor still mints. Vacant cap stays `mandate.occurrence_fresh`. A floor above the lid stays `mandate.range_fresh`. Lid TAP is hire-time max (`payment.amount_range`). Hatch TAP is a closed hatch. A closed coffer is `mandate.budget_fresh`. A mixed envelope is `mandate.currency_fresh`. A cap below the desk is `mandate.cap_fresh`. |
| 102 | `mandate.cap_fresh` | issue_intent with aether.max_autonomy and a known subject | named subject's live rung is above max | — | subject's live rung is at or below the cap. Exact cap still mints. An open ceiling (omit the constraint) still mints. Vacant cap stays `mandate.occurrence_fresh`. A closed hatch stays `mandate.lid_fresh`. Ghost subject stays `identity.known`. Hire still names `ladder.max_autonomy_constraint`. Ceiling TAP is a climb after mint. Grade TAP is a junior nested mint. Eave TAP is a cap below the desk at mint. A grant below the desk is `kya.grant_fresh`. |
| 103 | `kya.grant_fresh` | kya.attest with a numeric maxAutonomy and a known delegate | named delegate's live rung is above max | — | delegate's live rung is at or below the grant. Exact grant still mints. An open ceiling (omit maxAutonomy) still mints as L5. Ghost delegate stays `identity.known`. A second live hop stays `kya.unique_live`. A corpse mint stays `kya.mint_fresh`. A century mint stays `kya.mint_window`. An agent over-grant stays `kya.capability_subset`. Hire still names `kya.capability_subset`. Climb TAP is a climb after mint. Eave TAP is a slip cap below the desk. Sill TAP is a grant below the desk at mint. A nested grant wider than its parent is `kya.nest_tighter`. |
| 104 | `kya.nest_tighter` | kya.attest with a known live parentId | child maxAutonomy (omit = L5) is above the parent hop's maxAutonomy | — | nested grant is at or below the parent hop. Exact match still mints. A tighter child still mints. Ghost parent stays `kya.known_parent`. Dead parent stays `kya.parent_fresh`. A grant below the desk stays `kya.grant_fresh`. A second live hop stays `kya.unique_live`. An agent over-grant stays `kya.capability_subset`. Mandate `child_tighter` is a nested slip. Joist TAP is a nested grant wider than its parent at mint. A grant wider than the incoming hop is `kya.path_tighter`. A nested hop under another principal is `kya.nest_party`. |
| 105 | `kya.path_tighter` | kya.attest naming a principal other than the speaker, when the speaker has a live path from that principal | child maxAutonomy (omit = L5) is above the leaf incoming hop's maxAutonomy | — | path grant is at or below the incoming hop. Exact match still mints. A tighter child still mints. Speaker granting in their own name is not this deny. No live path does not set this flag (`kya.path_live`). A nested grant wider than its parent stays `kya.nest_tighter`. A grant below the desk stays `kya.grant_fresh`. An agent over-grant stays `kya.capability_subset`. Well TAP equal hops still mint. Stud TAP is a grant wider than the incoming hop at mint. An orphan hop is `kya.path_live`. |
| 106 | `kya.path_live` | kya.attest naming a known principal other than the speaker, when the speaker has no live path from that principal | no live incoming hop | — | speaker has a live path from that principal. Speaker granting in their own name is not this deny. Ghost principal stays `identity.known`. An agent filling in another principal's id stays `kya.party`. A grant wider than a live incoming hop stays `kya.path_tighter`. A nested grant wider than its parent stays `kya.nest_tighter`. A grant below the desk stays `kya.grant_fresh`. A dead parentId stays `kya.parent_fresh`. Well TAP equal hops still mint. Plate TAP is an orphan hop at mint. A nested hop under another principal is `kya.nest_party`. |
| 107 | `mandate.child_currency` | issue_intent with a known parent and amount_range or payment.budget | child's matching lid or coffer names a different currency than the parent | — | nested lid and coffer name the parent's currency, or omit a constraint the parent also omitted. Matching USD still mints. Matching USDC still mints. Same-slip lid vs coffer stays `mandate.currency_fresh`. A wider nested slip stays `mandate.child_tighter`. Ghost parent stays `mandate.known_parent`. Dead parent stays `mandate.parent_fresh`. A closed hatch stays `mandate.lid_fresh`. Hire still names `payment.currency_match`. Clash TAP is a mixed envelope. Header TAP is a nested child in a different currency. |
| 108 | `market.payout_fresh` | quote with an `fx` object | floor(from * rateE6 / 1e6) is 0 (missing/non-finite amount or rate do not set this flag) | — | floor payout is at least 1 minor unit. A 2-cent window at the low band still mints. A 1-cent window at par still mints. A 200bps miss stays `mm.spread_bound`. A dead window stays `market.fx_fresh`. A swapped pair stays `market.fx_pair`. A missing window stays `market.fx_window`. Ghost RFQ stays `market.known_rfq`. A research quote with no `fx` is not this deny. A vendor conversion while a maker sits is `market.fx_party`. An empty pit does not waive the band (`market.rate_fresh`). Pip TAP is a conversion that pays nothing. |
| 109 | `market.fx_party` | quote with an `fx` object | speaker is not `market_maker` and a `market_maker` already sits | — | speaker is the market maker, or no maker sits. Quoting FX with no maker on the pit is not this deny (Maker TAP still mints; settle stays `mm.known`). A closed guest list stays `market.invited_seller`. A 200bps miss stays `mm.spread_bound`. A conversion that pays nothing stays `market.payout_fresh`. A dead window stays `market.fx_fresh`. A swapped pair stays `market.fx_pair`. A missing window stays `market.fx_window`. Ghost RFQ stays `market.known_rfq`. A research quote with no `fx` is not this deny. An empty pit does not waive the band (`market.rate_fresh`). Quoin TAP is a vendor's conversion while a maker sits. |
| 110 | `market.rate_fresh` | quote with an `fx` object | nested `rateE6` sits outside 980000–1020000 (missing/non-finite rate does not set this flag) | — | nested rate is inside the 200bps band. A maker's own off-band quote stays `mm.spread_bound`. A vendor conversion while a maker sits stays `market.fx_party`. A conversion that pays nothing stays `market.payout_fresh`. A dead window stays `market.fx_fresh`. A swapped pair stays `market.fx_pair`. A missing window stays `market.fx_window`. Ghost RFQ stays `market.known_rfq`. An in-band guest quote with no maker still mints. The maker still mints in-band after sitting. Ashlar TAP is an empty pit that does not waive the band. |
| 111 | `kya.nest_party` | kya.attest with a known live parentId | nested hop principal is not the parent hop's principal | — | nested hop names the parent's principal. Exact same-principal nest still mints. A tighter same-principal nest still mints. Speaker granting in their own name without parentId is not this deny. Ghost parent stays `kya.known_parent`. Dead parent stays `kya.parent_fresh`. A nested grant wider than its parent stays `kya.nest_tighter`. An orphan hop stays `kya.path_live`. An agent filling in another principal's id stays `kya.party`. A grant below the desk stays `kya.grant_fresh`. Corbel TAP is a nested hop under another principal at mint. |
| 112 | `mandate.checkout_party` | issue_cart / issue_payment | speaker is not the hire buyer or intent subject, and is not a human or treasury | — | speaker is the hire buyer, intent subject, or a kill-switch role. Ghost hire stays `hire.known`. Ghost cart stays `mandate.known_cart`. A cheaper cart stays `hire.cart_matches`. A second cart stays `hire.unique_cart`. A second payment stays `mandate.unique_payment`. Dump stays `mandate.cart_party`. Spike stays `mandate.payment_party`. Trolley TAP is a second desk filling the research desk's unused checkout. Completing funded work is legal. Buyer still fills its own checkout. Human/treasury still fill. Poach TAP is hiring from someone else's room (`hire.room_party`). |
| 113 | `hire.room_party` | hire.create | speaker is not the RFQ buyer, and is not a human or treasury | — | speaker is the named buyer or a kill-switch role. Ghost quote stays `market.known_rfq`. A spent quote stays `hire.quote_unspent`. A shut or expired room stays `market.not_expired`. An FX window stays `hire.not_fx`. Shut TAP is tearing the room (`market.rfq_party`). Fold TAP is tearing the bid (`market.party`). Poach TAP is a second desk hiring the research desk's live unused quote. Completing funded work is legal. Buyer still hires its own quote. Human/treasury still hire. A foreign room plus a foreign slip stays this first. Guise TAP is wearing someone else's unused slip (`hire.slip_party`). |
| 114 | `hire.slip_party` | hire.create | speaker is not the intent subject, and is not a human or treasury | — | speaker is the named subject or a kill-switch role. Ghost intent stays `mandate.known_intent`. A ripped unused slip stays `mandate.not_expired`. A foreign room stays `hire.room_party`. Rip TAP is tearing the slip (`mandate.party`). Subject TAP is fund/submit (`mandate.subject_is_actor`). Guise TAP is a second desk hiring against the research desk's live unused slip in its own room. Completing funded work is legal. Named subject still hires. Human/treasury still hire. |
| 115 | `mandate.child_party` | issue_intent with a known parentId | speaker is not the parent subject or issuer, and is not a human or treasury | — | speaker is the parent subject, issuer, or a kill-switch role. Ghost parent stays `mandate.known_parent`. A dead parent stays `mandate.parent_fresh`. A wider nested slip stays `mandate.child_tighter`. A junior nested mint stays `ladder.min_level`. A mixed nested currency stays `mandate.child_currency`. Grade TAP is L3 (`ladder.min_level`). Header TAP is mixed currency (`mandate.child_currency`). Sub-hire TAP is the parent subject nesting. Cuckoo TAP is a second desk nesting under the research desk's live unused parent. Completing funded work is legal. Parent subject still nests. Human/treasury still nest. Minting a root in someone else's name is `mandate.root_party`. |
| 116 | `mandate.root_party` | issue_intent without parentId, when the named subject exists | speaker is not the named subject, and is not a human or treasury | — | speaker is the named subject or a kill-switch role. Ghost subject stays `identity.known`. Nested foreign stays `mandate.child_party`. A junior foreign root stays `ladder.min_level`. A vendor root stays `actor.role_capability`. Warrant TAP is a self-root. Cuckoo TAP is a nested foreign child. Sub-hire TAP is the parent subject nesting. Forge TAP is a second desk minting a root in the research desk's name. Completing funded work is legal. Named subject still mints a self-root. Human/treasury still mint roots for a desk. |
| 117 | `market.settle_party` | fx_settle when the quote has `fx` | speaker is not the named vendor seller, or the speaker is the market maker (a wash is not a conversion) | — | speaker is this window's converter, or a kill-switch role. A maker's quoted window still converts for any non-maker with books. Ghost / paper / spent quotes stay `market.fx_quote`. A missing maker stays `mm.known`. A missing dest book stays `ledger.known_account`. A desk settle stays `actor.role_capability`. Maker TAP is settle with no MM (`mm.known`). Wallet TAP is a missing dest book (`ledger.known_account`). Paper TAP is a research quote (`market.fx_quote`). Quoin TAP is minting FX while a maker sits (`market.fx_party`). Fold TAP is tearing a bid (`market.party`). Snare TAP is a second vendor settling the research vendor's unused conversion window. Completing funded work is legal. Named seller still converts its own window. A maker window still converts. Human/treasury still pass the party flag. |
| 118 | `mm.fx_only` | market.quote of a listed SKU when the speaker is a market_maker | the quoted SKU is not an FX SKU | — | a maker's quote is a window, not a good. Makers have no `hire.accept` / `hire.deliver` verb, so a maker's good quote would mint a hire no verb can advance. Vendors still quote goods. Makers still quote FX windows. A ghost room stays `market.known_rfq`. A shut room stays `market.not_expired`. An uninvited maker stays `market.invited_seller`. A windowless FX quote stays `market.fx_window` (Pane TAP). A vendor minting FX while a maker sits stays `market.fx_party` (Quoin TAP). Hawk TAP is a maker quoting the listed research good. Completing funded work is legal. |
| 119 | `kya.revoke_state` | kya.revoke | the revoke would tombstone no unrevoked hop (including cascades) and block no new pair: the named handshake is already revoked, the named pair is already blocked with nothing left, or the revoke names neither an attestation nor a delegate | — | a tombstone is not a second tombstone; a no-op revoke is not a notary line after yes. Re-attest, then revoke, writes a fresh tombstone. A ghost handshake stays `kya.known_attestation` (Seal TAP). A ghost agent stays `identity.known`. Someone else's name stays `kya.party` (Name TAP). A first pair-wide revoke still blocks implicit grants. An expired unrevoked hop still tombstones (it still occupies `kya.unique_live`, Twin TAP). Thaw TAP is the freeze analog (`identity.freeze_state`). Tomb TAP is a second tombstone. |
| 120 | `clearing.settle_state` | clearing.settle_window | the named currency's open exposure book has zero legs — the settle would photograph nothing, empty nothing, and still mint a window object plus a CLEARING_WINDOW notary line | — | an empty book is not a settlement photo; a no-op settle is not a notary line after yes. Fund a hire or settle an FX window to put gross on the book, then settle. The other currency's legs are a different book. A system speaker stays `actor.system_scope`. A junior speaker stays `actor.role_capability`. A garbage currency stays `command.malformed` before policy. Coffer TAP is the bilateral cap (`clearing.bilateral_limit`). Thaw and Tomb TAPs are the freeze and revoke analogs. Film TAP is a photo of nothing. |
...
| Quote / hire as seller | no | no | yes | FX only (`mm.fx_only` at quote) | no | no |
...
**Fixture:** `fixtures/demo/shut/`.

An $800 hire funds. A second desk's `market.close` of the research desk's live RFQ is `market.rfq_party` — a missing room and an expired window still allow; `market.party` still allows — and writes no `RFQ_CLOSE` line. The room stays live. The buyer still shuts its own room. Hiring that shut room's quote is `market.not_expired`. That funded work still releases. Someone else's room is not yours to close. Ghost shut stays `market.known_rfq`. Closing the room does not kill an already-minted FX window. Inspect labels `closed` without writing status into the store. Closed wins over expired. Fold TAP is someone else's bid. Rip TAP is someone else's unused slip. Now 93 rules.

### Dump (`pnpm demo dump`)

**Fixture:** `fixtures/demo/dump/`.

An $800 hire funds. A second desk's `mandate.revoke_cart` of the research desk's unused checkout is `mandate.cart_party` — a missing cart and an expired window still allow; `mandate.party` still allows — and writes no `CART_REVOKE` line. The cart stays live. The buyer still dumps its own unused cart. Paying that dumped cart is `mandate.not_expired`. Occupancy frees. That funded work still releases. Someone else's unused checkout is not yours to dump. Ghost dump stays `mandate.known_cart`. Bound is when a payment occupies it. Completing funded work is legal. Inspect labels `revoked` without writing status into the store. Bound wins over revoked. Revoked wins over expired. Rip TAP is someone else's unused slip. Shut TAP is someone else's room. Now 94 rules.

### Spike (`pnpm demo spike`)

**Fixture:** `fixtures/demo/spike/`.

An $800 hire funds. A second desk's `mandate.revoke_payment` of the research desk's unused check is `mandate.payment_party` — a missing payment and an expired window still allow; `mandate.cart_party` still allows — and writes no `PAYMENT_REVOKE` line. The payment stays live. The buyer still spikes its own unused payment. Funding that spiked payment is `mandate.not_expired`. Occupancy frees. That funded work still releases. Someone else's unused payment is not yours to spike. Ghost spike stays `mandate.known_payment`. Funded is when escrow occupies it. Completing funded work is legal. Inspect labels `revoked` without writing status into the store. Funded wins over revoked. Revoked wins over expired. Dump TAP is someone else's unused checkout. Rip TAP is someone else's unused slip. Now 96 rules.

### Week (`pnpm demo week`)

**Fixture:** `fixtures/demo/week/`.

An $800 hire funds. Minting WEEKLY or MONTHLY that cannot admit a second hire before the seven-day exp is `mandate.cadence_reach` — a vacant slot, a closed calendar, and hire-time recurrence still allow — and writes no slip. A one-shot WEEKLY still mints. DAILY still mints. That funded work still releases. A week is not a cadence on a seven-day slip. Vacant TAP is no slots. Reach TAP is a calendar that opens after the slip dies. Daily TAP is 24 hours. Now 97 rules.

### Gulf (`pnpm demo gulf`)

**Fixture:** `fixtures/demo/gulf/`.

An $800 hire funds. Minting an `amount_range` whose min exceeds max is `mandate.range_fresh` — a vacant slot, a week on a seven-day slip, and hire-time max still allow — and writes no slip. An exact band still mints. An open floor still mints. That funded work still releases. A floor above the lid is not a range. Vacant TAP is no slots. Week TAP is WEEKLY/MONTHLY. Lid TAP is hire-time max. Now 98 rules.

### Coffer (`pnpm demo coffer`)

**Fixture:** `fixtures/demo/coffer/`.

An $800 hire funds. Minting a closed coffer (`payment.budget` max 0) or a coffer below the amount_range floor is `mandate.budget_fresh` — a vacant slot, a floor above the lid, and hire-time envelope still allow — and writes no slip. A coffer that covers the floor still mints. An open floor still mints. That funded work still releases. A closed coffer is not a budget. Vacant TAP is no slots. Gulf TAP is an inverted band. Purse TAP is hire-time envelope. Now 99 rules.

### Clash (`pnpm demo clash`)

**Fixture:** `fixtures/demo/clash/`.

An $800 hire funds. Minting an `amount_range` and a `payment.budget` in different currencies is `mandate.currency_fresh` — a vacant slot, a closed coffer, and hire-time currency still allow — and writes no slip. Matching USD still mints. Matching USDC still mints. That funded work still releases. A USDC coffer on a USD lid is not a budget. Vacant TAP is no slots. Coffer TAP is a closed envelope. Gulf TAP is an inverted band. Ink TAP is cart vs hire. Mix TAP is a mixed journal. Now 100 rules.

### Hatch (`pnpm demo hatch`)

**Fixture:** `fixtures/demo/hatch/`.

An $800 hire funds. Minting an `amount_range` whose max cannot admit a positive hire (max 0, or an exact zero lid) is `mandate.lid_fresh` — a vacant slot, a floor above the lid, and hire-time max still allow — and writes no slip. A live lid still mints. An open floor still mints. That funded work still releases. A closed hatch is not a range. Vacant TAP is no slots. Gulf TAP is an inverted band. Lid TAP is hire-time max. Now 101 rules.

### Eave (`pnpm demo eave`)

**Fixture:** `fixtures/demo/eave/`.

An $800 hire funds. Minting an `aether.max_autonomy` below the named subject's live rung is `mandate.cap_fresh` — a vacant slot, a closed hatch, and hire-time climb still allow — and writes no slip. An exact cap still mints. An open ceiling still mints. That funded work still releases. A cap below the desk is not a cap. Vacant TAP is no slots. Hatch TAP is a closed lid. Ceiling TAP is a climb after mint. Grade TAP is a junior nested mint. Now 102 rules.

### Sill (`pnpm demo sill`)

**Fixture:** `fixtures/demo/sill/`.

An $800 hire funds. Minting a `kya.attest` whose `maxAutonomy` is below the named delegate's live rung is `kya.grant_fresh` — a dead handshake, a second live hop, and hire-time climb still allow — and writes no hop. An exact grant still mints. An open ceiling still mints. That funded work still releases. A grant below the desk is not a handshake. Spark TAP is a corpse mint. Year TAP is a hop past one year. Pair TAP is a second live hop. Climb TAP is a climb after mint. Eave TAP is a slip cap below the desk. Now 103 rules.

### Joist (`pnpm demo joist`)

**Fixture:** `fixtures/demo/joist/`.

An $800 hire funds. Minting a nested `kya.attest` whose `maxAutonomy` is above the live parent hop (omit is L5) is `kya.nest_tighter` — a dead parent hop, a grant below the desk, and a nested slip still allow — and writes no hop. An exact nested grant still mints. A tighter nested grant still mints. That funded work still releases. A nested grant wider than its parent is not a handshake. Nest TAP is a dead parent hop. Graft TAP is a missing hop parent. Sill TAP is a grant below the desk. Climb TAP is a climb after mint. Pair TAP is a second live hop. Sub-hire TAP is a wider nested slip. Now 104 rules.

### Stud (`pnpm demo stud`)

**Fixture:** `fixtures/demo/stud/`.

An $800 hire funds. Minting a `kya.attest` in another principal's name whose `maxAutonomy` is above the speaker's live incoming hop (omit is L5) is `kya.path_tighter` — a nested parent hop, a grant below the desk, and a climb after mint still allow — and writes no hop. An exact path grant still mints. A tighter path grant still mints. That funded work still releases. A grant wider than the incoming hop is not a handshake. Well TAP is a same-max human chain. Joist TAP is a nested grant wider than its parent. Sill TAP is a grant below the desk. Climb TAP is a climb after mint. Name TAP is whose name a handshake is in. An orphan hop is `kya.path_live`. Now 105 rules.

### Plate (`pnpm demo plate`)

**Fixture:** `fixtures/demo/plate/`.

An $800 hire funds. Minting a `kya.attest` in another principal's name when the speaker has no live incoming path is `kya.path_live` — a grant wider than the incoming hop, a nested parent hop, and whose name a handshake is in still allow — and writes no hop. An exact path grant still mints. A tighter path grant still mints. That funded work still releases. An orphan hop is not a handshake. Stud TAP is a grant wider than the incoming hop. Joist TAP is a nested grant wider than its parent. Name TAP is whose name a handshake is in. Well TAP is a same-max human chain. Now 106 rules.

### Header (`pnpm demo header`)

**Fixture:** `fixtures/demo/header/`.

An $800 hire funds. Minting a nested USDC slip under a USD parent is `mandate.child_currency` — a mixed envelope, a wider nested slip, and hire-time currency still allow — and writes no slip. Matching USD still mints. Matching USDC still mints. That funded work still releases. A USDC header under a USD plate is not a nested slip. Clash TAP is a mixed envelope at mint. Sub-hire TAP is a wider nested slip. Ink TAP is cart vs hire. Now 107 rules.

### Pip (`pnpm demo pip`)

**Fixture:** `fixtures/demo/pip/`.

An $800 hire funds. Minting an FX window whose floor payout is 0 is `market.payout_fresh` — a dead window, a swapped pair, and a 200bps band miss still allow — and writes no quote. A two-cent window at the low band still mints. A one-cent window at par still mints. That two-cent window still converts. That funded work still releases. A conversion that pays nothing is not an FX window. Band TAP is a 200bps miss. Born TAP is a dead window. Swap TAP is a swapped pair. Pane TAP is a missing window. Hall TAP is a ghost RFQ. Now 108 rules.

### Quoin (`pnpm demo quoin`)

**Fixture:** `fixtures/demo/quoin/`.

An $800 hire funds. Minting an FX window as a vendor while a market maker sits is `market.fx_party` — a closed guest list, a 200bps band miss, and a conversion that pays nothing still allow — and writes no quote. The market maker still mints an in-band window. A par window still mints. That in-band window still converts. That funded work still releases. A vendor's conversion is not a market-maker window. Room TAP is an uninvited seller. Band TAP is a 200bps miss. Pip TAP is a conversion that pays nothing. Maker TAP is settle with no MM. Now 109 rules.

### Ashlar (`pnpm demo ashlar`)

**Fixture:** `fixtures/demo/ashlar/`.

An $800 hire funds. Minting an FX window at half price with no maker is `market.rate_fresh` — a 200bps miss by the maker, a vendor conversion while a maker sits, and a conversion that pays nothing still allow — and writes no quote. The market maker still mints an in-band window after sitting. A par window still mints. That in-band window still converts. That funded work still releases. An empty pit does not waive the band. Band TAP is a maker's own off-band quote. Quoin TAP is a vendor conversion while a maker sits. Pip TAP is a conversion that pays nothing. Maker TAP is settle with no MM. Now 110 rules.

### Corbel (`pnpm demo corbel`)

**Fixture:** `fixtures/demo/corbel/`.

An $800 hire funds. Minting a nested hop under another principal's parent hop is `kya.nest_party` — a nested grant wider than its parent, a dead parent hop, and whose name a handshake is in still allow — and writes no hop. An exact same-principal nested grant still mints. A tighter same-principal nested grant still mints. That funded work still releases. A nested hop under another principal is not a nested handshake. Joist TAP is a nested grant wider than its parent. Nest TAP is a dead parent hop. Graft TAP is a missing hop parent. Plate TAP is an orphan hop. Name TAP is whose name a handshake is in. Now 111 rules.


### Trolley (`pnpm demo trolley`)

**Fixture:** `fixtures/demo/trolley/`.

An $800 hire funds. A second desk's `mandate.issue_cart` of the research desk's unused checkout is `mandate.checkout_party` — a missing cart, dumping someone else's cart, and a second cart on the same hire still allow — and writes no cart. Filling that cart as a payment is the same deny and writes no payment. The buyer still fills its own checkout. That funded work still releases. Someone else's checkout is not yours to fill. Dump TAP is tearing an unused cart (`mandate.cart_party`). Spike TAP is tearing an unused payment (`mandate.payment_party`). Cart TAP is occupancy (`hire.unique_cart`). Completing funded work is legal. Now 112 rules.

### Poach (`pnpm demo poach`)

**Fixture:** `fixtures/demo/poach/`.

An $800 hire funds. A second desk's `hire.create` of the research desk's live unused quote is `hire.room_party` — a missing room, a spent quote, and shutting someone else's room still allow — and writes no hire. The buyer still hires its own quote. That funded work still releases. Someone else's room is not yours to hire from. Shut TAP is tearing the room (`market.rfq_party`). Fold TAP is tearing the bid (`market.party`). Completing funded work is legal. Now 113 rules.

### Guise (`pnpm demo guise`)

**Fixture:** `fixtures/demo/guise/`.

An $800 hire funds. A second desk's `hire.create` against the research desk's live unused slip, in the second desk's own room, is `hire.slip_party` — a missing slip, a ripped slip, and hiring from someone else's room still allow — and writes no hire. The named subject still hires against its own unused slip. That funded work still releases. Someone else's unused slip is not yours to hire against. Rip TAP is tearing the slip (`mandate.party`). Poach TAP is hiring from someone else's room (`hire.room_party`). Subject TAP is fund/submit (`mandate.subject_is_actor`). Completing funded work is legal. Now 114 rules.

### Cuckoo (`pnpm demo cuckoo`)

**Fixture:** `fixtures/demo/cuckoo/`.

An $800 hire funds. A second desk's `mandate.issue_intent` nested under the research desk's live unused parent is `mandate.child_party` — a missing parent, a dead parent, a wider nested slip, and a junior nested mint still allow — and writes no child. The parent subject still nests a tighter child. That funded work still releases. Someone else's parent slip is not yours to nest under. Grade TAP is L3 (`ladder.min_level`). Header TAP is mixed currency (`mandate.child_currency`). Sub-hire TAP is the parent subject nesting. Completing funded work is legal. Now 115 rules.

### Forge (`pnpm demo forge`)

**Fixture:** `fixtures/demo/forge/`.

An $800 hire funds. A second desk's `mandate.issue_intent` without `parentId` naming the research desk is `mandate.root_party` — a missing subject, a nested child, a junior mint, and a vendor verb still allow — and writes no extra root. The named subject still mints a self-root. That funded work still releases. Someone else's name is not a root slip to mint. Cuckoo TAP is a nested foreign child (`mandate.child_party`). Warrant TAP is a self-root. Grade TAP is L3 (`ladder.min_level`). Sub-hire TAP is the parent subject nesting. Completing funded work is legal. Now 116 rules.

### Snare (`pnpm demo snare`)

**Fixture:** `fixtures/demo/snare/`.

An $800 hire funds. A second vendor's `market.fx_settle` of the research vendor's unused conversion window is `market.settle_party` — a missing maker, a missing quote, a missing dest book, and a vendor verb still allow — and does not consume the window. A maker wash-settling that window is the same refuse. The named seller still converts its own window. A maker window still converts. That funded work still releases. Someone else's conversion window is not yours to settle. Maker TAP is settle with no MM (`mm.known`). Wallet TAP is a missing dest book (`ledger.known_account`). Paper TAP is a research quote (`market.fx_quote`). Quoin TAP is minting FX while a maker sits (`market.fx_party`). Fold TAP is tearing a bid (`market.party`). Completing funded work is legal. Now 117 rules.

### Hawk (`pnpm demo hawk`)

**Fixture:** `fixtures/demo/hawk/`.

An $800 hire funds. A market maker's `market.quote` of the listed research good is `mm.fx_only` — a ghost room, a shut room, an uninvited maker, and a windowless FX quote still allow first-deny elsewhere — and writes no quote. Makers have no `hire.accept` / `hire.deliver` verb, so a maker's good quote would mint a hire no verb can advance. The research vendor still quotes the good. The maker still quotes an FX window, and that window still converts. That funded work still releases. A maker's quote is a window, not a good. Pane TAP is a windowless FX quote (`market.fx_window`). Quoin TAP is a vendor minting FX while a maker sits (`market.fx_party`). Completing funded work is legal. Now 118 rules.

### Tomb (`pnpm demo tomb`)

**Fixture:** `fixtures/demo/tomb/`.

An $800 hire funds. A first tombstone writes. `kya.revoke` of the already-revoked handshake is `kya.revoke_state` — a ghost handshake is not this deny, a ghost agent is not this deny, someone else's name is not this deny, a first pair-wide revoke is not this deny — and writes no `KYA_REVOKE` line. The pair-wide revoke of the already-blocked pair and the bare revoke naming nothing are the same refuse. The founder still re-attests and revokes the fresh hop. A first pair-wide revoke of a never-attested pair still blocks implicit grants. That funded work still releases. A tombstone is not a second tombstone. Seal TAP is a ghost handshake (`kya.known_attestation`). Name TAP is whose name a handshake is in (`kya.party`). Thaw TAP is the freeze analog (`identity.freeze_state`). Completing funded work is legal. Now 119 rules.

### Film (`pnpm demo film`)

**Fixture:** `fixtures/demo/film/`.

An $800 hire funds — one open leg on the USD book; the USDC book is empty. `clearing.settle_window` of the empty USDC book is `clearing.settle_state` — a system speaker is not this deny, a junior speaker is not this deny — and mints no window and no `CLEARING_WINDOW` line. The funded USD book still settles once (one leg consumed, $800 gross). The second photo right after is the same refuse. That funded work still releases. An empty book is not a settlement photo. Coffer TAP is the bilateral cap (`clearing.bilateral_limit`). Thaw and Tomb TAPs are the freeze and revoke analogs. Money already moved at escrow — a window is the CCP photo, not a second payment. Completing funded work is legal. Now 120 rules.
...
| `recurrence.test.ts` | First hire with `max_occurrences: 1` then a second create is `payment.recurrence`; DAILY gap binds until 24h; minting `max_occurrences` ≤ 0 is `mandate.occurrence_fresh`, not a written corpse; one slot still mints; WEEKLY max 8 is `mandate.cadence_reach`; one-shot WEEKLY still mints; ghost subject stays `identity.known`; ghost parent stays `mandate.known_parent` |
...
| `rfq-party.test.ts` | Shut TAP; $800 hire funds; market.close of a live RFQ by a second desk is `market.rfq_party` and writes no RFQ_CLOSE line; known_rfq, not_expired, role, and market.party still allow; buyer still shuts its own room; hire.create of that shut room's quote is `market.not_expired`; funded work still releases |
| `market.test.ts` | Catalog SKU deny; stale quote cannot be hired; audit.query by hire id; garbage role/decision/numeric agentId is `command.malformed`; a USD-only SKU quoted in USDC is `market.sku_currency`; `fxPairSettles` is the FX SKU priced in `from`; inviting a missing agent onto an RFQ is `identity.known`, not a closed room; ghost close is `market.known_rfq`; stranger close is `market.rfq_party`; hire.create after close is `market.not_expired`; stranger hire.create of a live unused quote is `hire.room_party`; treasury still hires; spent plus foreign stays `hire.quote_unspent` first; stranger hire.create against a live unused slip in its own room is `hire.slip_party`; treasury still hires against another desk's unused slip; a foreign room plus a foreign slip stays `hire.room_party` first; a ghost slip stays `mandate.known_intent` first |
| `cart-party.test.ts` | Dump TAP; $800 hire funds; mandate.revoke_cart of a live unused cart by a second desk is `mandate.cart_party` and writes no CART_REVOKE line; known_cart, not_expired, role, mandate.party, market.party, and market.rfq_party still allow; buyer still dumps its own unused cart; issue_payment of that dumped cart is `mandate.not_expired`; occupancy frees; bound dump is `mandate.not_expired`; funded work still releases |
| `payment-party.test.ts` | Spike TAP; $800 hire funds; mandate.revoke_payment of a live unused payment by a second desk is `mandate.payment_party` and writes no PAYMENT_REVOKE line; known_payment, not_expired, role, mandate.cart_party, mandate.party, market.party, and market.rfq_party still allow; buyer still spikes its own unused payment; hire.fund of that spiked payment is `mandate.not_expired`; occupancy frees; funded spike is `mandate.not_expired`; funded work still releases |
| `cadence-reach.test.ts` | Week TAP; $800 hire funds; mandate.issue_intent WEEKLY/MONTHLY that cannot admit a second hire is `mandate.cadence_reach` and writes no slip; occurrence_fresh, window_reach, window_fresh, payment.recurrence, and child_tighter still allow; a one-shot WEEKLY still mints; DAILY still mints; funded work still releases |
| `range-fresh.test.ts` | Gulf TAP; $800 hire funds; mandate.issue_intent amount_range min exceeding max is `mandate.range_fresh` and writes no slip; occurrence_fresh, cadence_reach, window_reach, window_fresh, payment.amount_range, and child_tighter still allow; an exact band still mints; an open floor still mints; funded work still releases |
| `budget-fresh.test.ts` | Coffer TAP; $800 hire funds; mandate.issue_intent budget max 0 or budget below the amount_range floor is `mandate.budget_fresh` and writes no slip; occurrence_fresh, cadence_reach, range_fresh, payment.budget, payment.amount_range, and child_tighter still allow; a coffer that covers the floor still mints; an open floor still mints; funded work still releases |
| `currency-fresh.test.ts` | Clash TAP; $800 hire funds; mandate.issue_intent amount_range and payment.budget in different currencies is `mandate.currency_fresh` and writes no slip; occurrence_fresh, cadence_reach, range_fresh, budget_fresh, payment.currency_match, payment.budget, payment.amount_range, and child_tighter still allow; matching USD still mints; matching USDC still mints; funded work still releases |
| `hatch-fresh.test.ts` | Hatch TAP; $800 hire funds; mandate.issue_intent amount_range max 0 or an exact zero lid is `mandate.lid_fresh` and writes no slip; occurrence_fresh, cadence_reach, range_fresh, budget_fresh, currency_fresh, payment.amount_range, and child_tighter still allow; a live lid still mints; an open floor still mints; funded work still releases |
| `cap-fresh.test.ts` | Eave TAP; $800 hire funds; mandate.issue_intent aether.max_autonomy below the desk's live rung is `mandate.cap_fresh` and writes no slip; occurrence_fresh, cadence_reach, range_fresh, budget_fresh, currency_fresh, lid_fresh, ladder.max_autonomy_constraint, ladder.min_level, and child_tighter still allow; an exact cap still mints; an open ceiling still mints; funded work still releases |
| `grant-fresh.test.ts` | Sill TAP; $800 hire funds; kya.attest maxAutonomy below the desk's live rung is `kya.grant_fresh` and writes no hop; mint_fresh, mint_window, unique_live, not_self, party, identity.known, capability_subset, and cap_fresh still allow; an exact grant still mints; an open ceiling still mints; funded work still releases |
| `nest-tighter.test.ts` | Joist TAP; $800 hire funds; kya.attest with parentId and maxAutonomy above the parent hop is `kya.nest_tighter` and writes no hop; grant_fresh, parent_fresh, known_parent, unique_live, capability_subset, and child_tighter still allow; an exact nested grant still mints; a tighter nested grant still mints; funded work still releases |
| `path-tighter.test.ts` | Stud TAP; $800 hire funds; kya.attest in the founder's name with maxAutonomy above the speaker's live incoming hop is `kya.path_tighter` and writes no hop; nest_tighter, grant_fresh, unique_live, capability_subset, party, and path_live still allow; an exact path grant still mints; a tighter path grant still mints; funded work still releases |
| `path-live.test.ts` | Plate TAP; $800 hire funds; kya.attest in the founder's name with no live incoming hop is `kya.path_live` and writes no hop; path_tighter, nest_tighter, grant_fresh, unique_live, capability_subset, and party still allow; an exact path grant still mints; a tighter path grant still mints; funded work still releases |
| `child-currency.test.ts` | Header TAP; $800 hire funds; nested USDC under a USD parent is `mandate.child_currency` and writes no slip; currency_fresh, child_tighter, and payment.currency_match still allow; matching USD still mints; matching USDC still mints; funded work still releases |
| `payout-fresh.test.ts` | Pip TAP; $800 hire funds; FX 1¢ at the low band and 0¢ at par are `market.payout_fresh` and write no quote; fx_fresh, fx_window, fx_pair, not_expired, known_rfq, and spread_bound still allow; a two-cent window at the low band still mints; a one-cent window at par still mints; the two-cent window still converts; funded work still releases |
| `fx-maker.test.ts` | Quoin TAP; $800 hire funds; vendor and compute-vendor in-band FX while a maker sits are `market.fx_party` and write no quote; invited_seller, spread_bound, payout_fresh, fx_fresh, fx_window, fx_pair, and known_rfq still allow; the maker still mints an in-band window; a par window still mints; the in-band window still converts; funded work still releases |
| `rate-fresh.test.ts` | Ashlar TAP; $800 hire funds; vendor and compute-vendor off-band FX with no maker are `market.rate_fresh` and write no quote; spread_bound, fx_party, payout_fresh, invited_seller, fx_fresh, fx_window, and fx_pair still allow; the maker still mints an in-band window after sitting; a par window still mints; the in-band window still converts; funded work still releases |
| `nest-party.test.ts` | Corbel TAP; $800 hire funds; kya.attest with parentId whose principal is not the nested hop's principal is `kya.nest_party` and writes no hop; nest_tighter, parent_fresh, known_parent, unique_live, party, path_live, grant_fresh, and child_tighter still allow; an exact same-principal nested grant still mints; a tighter same-principal nested grant still mints; funded work still releases |
| `checkout-party.test.ts` | Trolley TAP; $800 hire funds; mandate.issue_cart of a live unused hire by a second desk is `mandate.checkout_party` and writes no cart; mandate.issue_payment of that cart by a second desk is `mandate.checkout_party` and writes no payment; unique_cart, cart_matches, known_cart, cart_party, payment_party, and party still allow; buyer still fills its own checkout; funded work still releases |
| `hire-room-party.test.ts` | Poach TAP; $800 hire funds; hire.create of a live unused quote by a second desk is `hire.room_party` and writes no hire; known_rfq, quote_unspent, not_expired, not_fx, no_self_deal, role, rfq_party, hire.party, subject_is_actor, checkout_party, and slip_party still allow; a missing room stays `market.known_rfq`; a shut room stays `market.not_expired`; buyer still hires its own quote; funded work still releases |
| `hire-slip-party.test.ts` | Guise TAP; $800 hire funds; hire.create against a live unused intent by a second desk is `hire.slip_party` and writes no hire; known_intent, room_party, quote_unspent, not_expired, not_fx, no_self_deal, role, hire.party, subject_is_actor, checkout_party, and mandate.party still allow; a missing slip stays `mandate.known_intent`; a ripped slip stays `mandate.not_expired`; a foreign room stays `hire.room_party`; named subject still hires against its own unused slip; funded work still releases |
| `child-party.test.ts` | Cuckoo TAP; $800 hire funds; mandate.issue_intent nested under another desk's live unused parent by a second desk is `mandate.child_party` and writes no child; known_parent, parent_fresh, child_tighter, child_currency, role, and min_level still allow; a missing parent stays `mandate.known_parent`; a dead parent stays `mandate.parent_fresh`; a wider nested slip stays `mandate.child_tighter`; a junior nested mint stays `ladder.min_level`; parent subject still nests a tighter child; funded work still releases |
| `root-party.test.ts` | Forge TAP; $800 hire funds; mandate.issue_intent without parentId naming another desk by a second desk is `mandate.root_party` and writes no extra root; known, child_party, role, and min_level still allow; a missing subject stays `identity.known`; a nested foreign child stays `mandate.child_party`; a junior foreign root stays `ladder.min_level`; a vendor root stays `actor.role_capability`; named subject still mints a self-root; funded work still releases |
| `settle-party.test.ts` | Snare TAP; $800 hire funds; market.fx_settle of another vendor's unused FX window by a second data vendor is `market.settle_party` and does not consume the window; fx_quote, mm.known, known_account, sufficient, and role still allow; a missing maker stays `mm.known`; a missing quote stays `market.fx_quote`; a missing dest book stays `ledger.known_account`; a desk settle stays `actor.role_capability`; a maker wash stays `market.settle_party`; named seller still converts its own window; a maker window still converts; funded work still releases |
| `fx-only.test.ts` | Hawk TAP; $800 hire funds; market.quote of the listed research good by a market maker is `mm.fx_only` and writes no quote; known_rfq, known_sku, not_expired, invited_seller, role, and spread_bound still allow; a ghost room stays `market.known_rfq`; a shut room stays `market.not_expired`; an uninvited maker stays `market.invited_seller`; a windowless maker FX quote stays `market.fx_window`; the research vendor still quotes the good; the maker still quotes an FX window and it still converts; funded work still releases |
| `revoke-state.test.ts` | Tomb TAP; $800 hire funds; a first tombstone writes; kya.revoke of the already-revoked handshake is `kya.revoke_state` and writes no `KYA_REVOKE` line; the pair-wide revoke of the blocked pair and the bare revoke are the same refuse; a ghost handshake stays `kya.known_attestation`; a ghost agent stays `identity.known`; someone else's name stays `kya.party`; re-attest then revoke still tombstones; a first pair-wide revoke still blocks; funded work still releases |
| `settle-state.test.ts` | Film TAP; $800 hire funds with one open USD leg; clearing.settle_window of the empty USDC book is `clearing.settle_state` and mints no window and no CLEARING_WINDOW line; a system speaker stays `actor.system_scope`; a junior speaker stays `actor.role_capability`; the funded USD book still settles once; the second photo is the same refuse; the other currency's legs are a different book; funded work still releases |
| `mcp.test.ts` | Sub-hire TAP + clearing-window TAP over `aether_demo_clearing` + refund TAP over `aether_demo_refund` + replay TAP over `aether_demo_replay` + envelope-nonce TAP over `aether_demo_nonce` + deny-cache TAP over `aether_demo_deny` + recurrence TAP over `aether_demo_recurrence` + calendar TAP over `aether_demo_calendar` + slot TAP over `aether_demo_slot` + daily TAP over `aether_demo_daily` + cart occupancy TAP over `aether_demo_cart` + velocity TAP over `aether_demo_velocity` + operator-door TAP over `aether_demo_door` + cart-match TAP over `aether_demo_match` + closed-room TAP over `aether_demo_room` + conversion TAP over `aether_demo_conversion` + unique-live TAP over `aether_demo_pair` + spread TAP over `aether_demo_band` + nest TAP over `aether_demo_nest` + heir TAP over `aether_demo_heir` + stock TAP over `aether_demo_stock` + purse TAP over `aether_demo_purse` + seat TAP over `aether_demo_seat` + cover TAP over `aether_demo_cover` + mint TAP over `aether_demo_mint` + payee TAP over `aether_demo_payee` + climb TAP over `aether_demo_climb` + born TAP over `aether_demo_born` + reach TAP over `aether_demo_reach` + year TAP over `aether_demo_year` + fuse TAP over `aether_demo_fuse` + sku TAP over `aether_demo_sku` + priced TAP over `aether_demo_priced` + party TAP over `aether_demo_party` + cash TAP over `aether_demo_cash` + stale TAP over `aether_demo_stale` + chain TAP over `aether_demo_chain` + arrow TAP over `aether_demo_arrow` + wallet TAP over `aether_demo_wallet` + name TAP over `aether_demo_name` + pane TAP over `aether_demo_pane` + subject TAP over `aether_demo_subject` + paper TAP over `aether_demo_paper` + mix TAP over `aether_demo_mix` + rung TAP over `aether_demo_rung` + grade TAP over `aether_demo_grade` + cradle TAP over `aether_demo_cradle` + ceiling TAP over `aether_demo_ceiling` + lapse TAP over `aether_demo_lapse` + pause TAP over `aether_demo_pause` + mirror TAP over `aether_demo_mirror` + warrant TAP over `aether_demo_warrant` + vacant TAP over `aether_demo_vacant` + badge TAP over `aether_demo_badge` + lid TAP over `aether_demo_lid` + bare TAP over `aether_demo_bare` + shelf TAP over `aether_demo_shelf` + hall TAP over `aether_demo_hall` + writ TAP over `aether_demo_writ` + crate TAP over `aether_demo_crate` + pact TAP over `aether_demo_pact` + root TAP over `aether_demo_root` + docket TAP over `aether_demo_docket` + graft TAP over `aether_demo_graft` + seal TAP over `aether_demo_seal` + guest TAP over `aether_demo_guest` + dust TAP over `aether_demo_dust` + thaw TAP over `aether_demo_thaw` + twin TAP over `aether_demo_twin` + fence TAP over `aether_demo_fence` + mute TAP over `aether_demo_mute` + nil TAP over `aether_demo_nil` + spark TAP over `aether_demo_spark` + wilt TAP over `aether_demo_wilt` + maker TAP over `aether_demo_maker` + ink TAP over `aether_demo_ink` + brim TAP over `aether_demo_brim` + swap TAP over `aether_demo_swap` + sour TAP over `aether_demo_sour` + cut TAP over `aether_demo_cut` + ice TAP over `aether_demo_ice` + rail TAP over `aether_demo_rail` + pen TAP over `aether_demo_pen` + well TAP over `aether_demo_well` + cite TAP over `aether_demo_cite` + lock TAP over `aether_demo_lock` + void TAP over `aether_demo_void` + fold TAP over `aether_demo_fold` + rip TAP over `aether_demo_rip` + shut TAP over `aether_demo_shut` + dump TAP over `aether_demo_dump` + spike TAP over `aether_demo_spike` + week TAP over `aether_demo_week` + gulf TAP over `aether_demo_gulf` + coffer TAP over `aether_demo_coffer` + clash TAP over `aether_demo_clash` + hatch TAP over `aether_demo_hatch` + eave TAP over `aether_demo_eave` + sill TAP over `aether_demo_sill` + joist TAP over `aether_demo_joist` + stud TAP over `aether_demo_stud` + plate TAP over `aether_demo_plate` + header TAP over `aether_demo_header` + pip TAP over `aether_demo_pip` + quoin TAP over `aether_demo_quoin` + ashlar TAP over `aether_demo_ashlar` + corbel TAP over `aether_demo_corbel` + trolley TAP over `aether_demo_trolley` + poach TAP over `aether_demo_poach` + guise TAP over `aether_demo_guise` + cuckoo TAP over `aether_demo_cuckoo` + forge TAP over `aether_demo_forge` + snare TAP over `aether_demo_snare` + MCP `tools/list` + `identity.register` replay + `aether_hire_refund`; command tools are 1:1 with CommandType including `aether_market_fx_settle` and `aether_ledger_transfer`; an unknown `actor` alias is `actor.known`, not silent system; omit or `system` still bootstraps; omit-actor `aether_audit_verify` is the same speaker as HTTP GET; omit-actor `aether_ledger_balances` / `aether_receipt_get` is the same speaker as HTTP GET; Demo tools are TAP runners |
...
| `market.test.ts` | Catalog SKU deny; stale quote cannot be hired; audit.query by hire id; garbage role/decision/numeric agentId is `command.malformed`; a USD-only SKU quoted in USDC is `market.sku_currency`; `fxPairSettles` is the FX SKU priced in `from`; inviting a missing agent onto an RFQ is `identity.known`, not a closed room; ghost close is `market.known_rfq`; stranger close is `market.rfq_party`; hire.create after close is `market.not_expired`; stranger hire.create of a live unused quote is `hire.room_party`; stranger hire.create against a live unused slip in its own room is `hire.slip_party`; stranger nested issue_intent under another desk's live unused parent is `mandate.child_party`; stranger root issue_intent in another desk's name is `mandate.root_party`; stranger fx_settle of another vendor's unused conversion window is `market.settle_party`; a maker quoting a research good is `mm.fx_only` (a ghost room, a shut room, an uninvited maker, and a windowless FX quote still first-deny elsewhere; vendors still quote goods; makers still quote FX windows) |
...
| `clearing.test.ts` | Pair netting; bilateral wouldExceed; settleWindow archives legs; `openLegs` counts one currency's open book; snapshot includes the instance cap |
