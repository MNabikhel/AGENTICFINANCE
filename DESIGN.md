...
| 07 | `payment.amount_range` | settle if constraint present | amount outside `[min,max]` or currency mismatch. Minting min > max is `mandate.range_fresh`. | — | in range. Lid TAP is hire-time max. Gulf TAP is a corpse band at mint. |
...
| 100 | `mandate.currency_fresh` | issue_intent with amount_range and payment.budget | lid and coffer name different currencies | — | same currency on both, or only one constraint present. Matching USD still mints. Matching USDC still mints. Vacant cap stays `mandate.occurrence_fresh`. A closed coffer stays `mandate.budget_fresh`. A floor above the lid stays `mandate.range_fresh`. Ink TAP is cart vs hire (`payment.currency_match`). Mix TAP is a mixed journal. Clash TAP is a mixed envelope at mint. |
