...
| 74 | `actor.system_scope` | always when actorId is system | command is not first-human bootstrap or a read (catalog / audit.query / balances / receipt.get / host.card) | — | system may bootstrap or read |
...
| 85 | `host.not_hosted` | host.subscribe | this instance is the public kernel (`hosted` is false) | — | a hosted operator (later). `host.card` is a read. Self-host is free. GitHub is not a checkout. |
...
| `policy.test.ts` | Table-driven: each ruleId has allow + deny fixtures; `velocity.window` escalates a new hire/fund/FX settle after a hot hour and allows complete-after-fund and reads; `host.not_hosted` refuses subscribe on the public kernel and allows `host.card` |
