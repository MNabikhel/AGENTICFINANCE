...
| 64 | `mandate.unique_payment` | issue_payment | cart already has a payment mandate (same cart hash) | — | cart has no payment yet. Inspect / snapshot label that cart `bound`, not `live`. |
