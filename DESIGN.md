...
| 39 | `market.known_rfq` | quote / hire.create | RFQ (or the quote’s RFQ) does not exist | — | room exists. Hall TAP is a ghost RFQ. Room TAP is a closed guest list. Shelf TAP is a ghost SKU. |
...
An $800 hire funds. `market.quote` on a ghost RFQ is `market.known_rfq` — a missing SKU still allows; a closed guest list is not this deny — and writes no quote. That funded work still releases. Room TAP is a closed guest list. Shelf TAP is a ghost SKU. Still 87 rules.
