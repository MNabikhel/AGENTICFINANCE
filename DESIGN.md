...
| `world.test.ts` | Durable boot restores keys and audit head; settlement window restores; constructor `bilateralLimit` is not in `world.json` |
...
The four TAP demos exist. Further work is sequenced in [`docs/FOUNDATION.md`](docs/FOUNDATION.md): adapter-shaped KYA issuers next. Do not add a UI. Do not add a live rail. Do not grind inspect overlays unless the pin would otherwise lie.
