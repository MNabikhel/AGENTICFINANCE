...
| `inspect.test.ts` | `aether_get` / inspect by id; MCP command schemas; expired approval tickets refuse resolve; inspect / snapshot label an expired ticket `expired`, not `pending`; a missing receipt is `receipt.known`, not an empty success |
...
| `kya.test.ts` | Nested hops; revoke cascades; unknown parent hop throws; unknown or foreign attestation throws; a second live pair throws; snapshot edges label expired hops `expired`, not `live`; revoked wins over expired |
