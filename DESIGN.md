...
An $800 hire funds under the auto-approve line. A $6,400 hire pauses as `approval.threshold`. After that ticket dies, `approval.resolve` is `approval.pending` — the ticket still exists; a stale command is not this deny — and writes no hire. The quote is free again. That funded work still releases. Velocity TAP is a hot hour. Deny TAP is a cached no. Replay TAP is a retry of an allow. Still 87 rules.
