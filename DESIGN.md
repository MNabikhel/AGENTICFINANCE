...
An $800 hire funds. `identity.register` on the taken desk alias is `identity.unique_key` — L5 at birth still allows; system minting a second agent is not this deny — and writes no agent. That funded work still releases. Cradle TAP is L5 at birth. Still 87 rules.
