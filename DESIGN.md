...
An $800 hire funds. `mandate.issue_payment` on a ghost cart is `mandate.known_cart` — occupancy still allows; a dead cart at fund is not this deny — and writes no payment. That funded work still releases. Cart TAP is occupancy. Chain TAP is a dead cart at fund. Writ TAP is a ghost slip. Still 87 rules.
