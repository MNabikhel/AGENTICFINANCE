...
export type MandateConstraint =
  ...
  | {
      type: "payment.budget";
      currency: CurrencyCode;
      max: number;            // remaining computed at eval. max ≤ 0, or max below an amount_range floor, is mandate.budget_fresh
    }
  | {
...
| 08 | `payment.budget` | settle if constraint present | `spentAgainstIntent + amount > max`. Minting max ≤ 0, or max below an amount_range floor, is `mandate.budget_fresh`. | — | remaining ≥ amount. Purse TAP is hire-time envelope. Coffer TAP is a closed envelope at mint. |
...
| 99 | `mandate.budget_fresh` | issue_intent with payment.budget | max ≤ 0, or max below an amount_range floor | — | envelope can still admit an amount the lid would allow. A coffer that covers the floor still mints. An open floor still mints. Vacant cap stays `mandate.occurrence_fresh`. A floor above the lid stays `mandate.range_fresh`. Purse TAP is hire-time envelope (`payment.budget`). Coffer TAP is a closed coffer. |
