...
Pin `aether.protocol.1` (`GET /v1/protocol`, resource `aether://protocol`, tool `aether_protocol`). `liveMoney` is `false` until adapters exist. Current card: `0.30.0`.
...
40. A transfer cannot overdraw the source book (`ledger.sufficient`). Draining to zero is legal. Negative cash is not.
