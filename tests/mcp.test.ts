...
describe("MCP host", () => {
  it("lists tools and dispatches identity.register", () => {
    ...
    expect(names).toContain("aether_demo_nil");
    expect(names).toContain("aether_demo_spark");
    expect(names).toContain("aether_demo_wilt");
    expect(names).toContain("aether_demo_maker");
    expect(names).toContain("aether_demo_ink");
    expect(names).toContain("aether_demo_brim");
    expect(names).toContain("aether_demo_swap");
    expect(names).toContain("aether_demo_sour");
    expect(names).toContain("aether_demo_cut");
    expect(names).toContain("aether_demo_ice");
    expect(names).toContain("aether_demo_rail");
    expect(names).toContain("aether_demo_pen");
    expect(names).toContain("aether_demo_well");
    expect(names).toContain("aether_demo_cite");
    ...
    expect(names).toContain("aether_mandate_revoke_payment");
  ...
  it("runs the nil demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_nil", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not an empty success");
  });

  it("runs the spark demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_spark", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("cannot be born dead");
  });

  it("runs the wilt demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_wilt", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("cannot be born with a closed calendar");
  });

  it("runs the maker demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_maker", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not a journal against nobody");
  });

  it("runs the ink demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_ink", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not the hire's money");
  });

  it("runs the brim demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_brim", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("IEEE rounding is not a mint");
  });

  it("runs the swap demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_swap", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not a silent journal of the books this rail actually posts");
  });

  it("runs the sour demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_sour", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not a late hire");
  });

  it("runs the cut demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_cut", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not an expiry");
  });

  it("runs the ice demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_ice", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not a frozen desk");
  });

  it("runs the rail demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_rail", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not decoration");
  });

  it("runs the pen demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_pen", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not a grown-up pause");
  });

  it("runs the well demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_well", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not a nested parent");
  });

  it("runs the cite demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_cite", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not decoration once a check exists");
  });

  it("runs the lock demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_lock", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not yours to turn");
  });

  it("runs the void demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_void", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not a refund");
  });

  it("runs the fold demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_fold", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
  ...
  it("runs the shut demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_shut", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
  ...
  it("runs the spike demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_spike", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
    expect(report.tldr).toContain("not yours to spike");
  });

  it("runs the week demo over the tool bus", () => {
    const mcp = new AetherMcp();
    const report = mcp.callTool("aether_demo_week", {}) as { ok: boolean; results: { ok: boolean }[]; tldr: string };
    expect(report.ok).toBe(true);
    expect(report.results.every((r) => r.ok)).toBe(true);
