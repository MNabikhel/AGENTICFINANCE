...
describe("market.fx_party", () => {
  it("refuses a vendor in-band FX window while a maker sits as fx_party, not a written corpse", () => {
    const rt = boot();
    const { desk, vendor, mm } = economy(rt);
    const rfq = must(
      rt.dispatch(
    ...
    expect(r.error.error.status).toBe(422);
    expect(r.error.error.type).toContain("policy.deny");
    expect(r.error.decision?.trace.find((t) => t.ruleId === "market.fx_party")?.verdict).toBe("deny");
    ...
    expect(r.error.decision?.trace.find((t) => t.ruleId === "market.fx_window")?.verdict).toBe("allow");
    expect(r.error.decision?.trace.find((t) => t.ruleId === "market.known_rfq")?.verdict).toBe("allow");
    expect(r.error.decision?.trace.find((t) => t.ruleId === "market.rate_fresh")?.verdict).toBe("allow");
    expect(r.error.decision?.remediation?.ruleId).toBe("market.fx_party");
    expect(r.error.decision?.remediation?.kind).toBe("none");
    expect(rt.quotes.size).toBe(before);
    expect(rt.clock.now()).not.toBe(clockBefore);
    expect(rt.audit.length).toBeGreaterThan(auditBefore);
  ...
  it("refuses a compute vendor in-band FX window while a maker sits as fx_party", () => {
    const rt = boot();
    const { desk, compute, mm } = economy(rt);
    const rfq = must(
      rt.dispatch(
        cmd("market.rfq", desk.id, {
          sku: "fx.usd_sim.usdc_sim",
          spec: "window",
          invitedSellerIds: [mm.id, compute.id],
        }),
      ),
      "fx rfq",
    );
  ...
  it("still mints when the market maker quotes", () => {
    const rt = boot();
    const { desk, mm } = economy(rt);
    const rfq = must(
      rt.dispatch(
    ...
    expect((quoted.data as { id: string }).id.startsWith("qte_")).toBe(true);
    expect(quoted.decision.trace.find((t) => t.ruleId === "market.fx_party")?.verdict).toBe("allow");
  });

  it("still names invited_seller first when the vendor is not invited", () => {
    const rt = boot();
    const { desk, vendor, mm } = economy(rt);
    const rfq = must(
      rt.dispatch(
    ...
    expect(r.ok).toBe(false);
    if (r.ok) return;
    expect(r.error.decision?.trace.find((t) => t.ruleId === "market.invited_seller")?.verdict).toBe("deny");
    expect(r.error.decision?.trace.find((t) => t.ruleId === "market.fx_party")?.verdict).toBe("deny");
    expect(r.error.decision?.remediation?.ruleId).toBe("market.invited_seller");
    expect(rt.quotes.size).toBe(before);
  });

  it("still names spread_bound first when the maker is off-band", () => {
    const rt = boot();
    const { desk, mm } = economy(rt);
    const rfq = must(
      rt.dispatch(
    ...
    if (r.ok) return;
    expect(r.error.decision?.trace.find((t) => t.ruleId === "mm.spread_bound")?.verdict).toBe("deny");
    expect(r.error.decision?.trace.find((t) => t.ruleId === "market.fx_party")?.verdict).toBe("allow");
    expect(r.error.decision?.trace.find((t) => t.ruleId === "market.rate_fresh")?.verdict).toBe("deny");
    expect(r.error.decision?.remediation?.ruleId).toBe("mm.spread_bound");
    expect(rt.quotes.size).toBe(before);
  });

  it("still names payout_fresh first when a vendor conversion pays nothing", () => {
    const rt = boot();
    const { desk, vendor, mm } = economy(rt);
    const rfq = must(
      rt.dispatch(
...
describe("market.rate_fresh", () => {
  function emptyPit() {
    ...
    rt.seedOpening({ "vendor:cash": { amount: 200_000, currency: "USD_SIM" } });
    return { rt, desk: rt.alias("procurement"), vendor: rt.alias("vendor"), compute: rt.alias("compute") };
  }

  it("refuses a vendor off-band FX window with no maker as rate_fresh, not a written corpse", () => {
    const { rt, desk, vendor } = emptyPit();
    expect([...rt.identity.all()].some((a) => a.role === "market_maker")).toBe(false);
    const rfq = must(
      rt.dispatch(cmd("market.rfq", desk.id, { sku: "fx.usd_sim.usdc_sim", spec: "window" })),
      "fx rfq",
    );
    const clockBefore = rt.clock.now();
    const auditBefore = rt.audit.length;
    ...
    expect(r.error.decision?.trace.find((t) => t.ruleId === "market.known_rfq")?.verdict).toBe("allow");
    expect(r.error.decision?.trace.find((t) => t.ruleId === "market.invited_seller")?.verdict).toBe("allow");
    expect(r.error.decision?.remediation?.ruleId).toBe("market.rate_fresh");
    expect(r.error.decision?.remediation?.kind).toBe("none");
    expect(rt.quotes.size).toBe(before);
    expect(rt.clock.now()).not.toBe(clockBefore);
    expect(rt.audit.length).toBeGreaterThan(auditBefore);
  ...
  it("refuses a compute vendor off-band FX window with no maker as rate_fresh", () => {
    const { rt, desk, compute } = emptyPit();
    const rfq = must(
      rt.dispatch(cmd("market.rfq", desk.id, { sku: "fx.usd_sim.usdc_sim", spec: "window" })),
      "fx rfq",
    );
  ...
  it("still mints an in-band vendor window when no maker sits", () => {
    const { rt, desk, vendor } = emptyPit();
    const rfq = must(
      rt.dispatch(cmd("market.rfq", desk.id, { sku: "fx.usd_sim.usdc_sim", spec: "window" })),
      "fx rfq",
    ...
    expect((quoted.data as { id: string }).id.startsWith("qte_")).toBe(true);
    expect(quoted.decision.trace.find((t) => t.ruleId === "market.rate_fresh")?.verdict).toBe("allow");
    expect(quoted.decision.trace.find((t) => t.ruleId === "market.fx_party")?.verdict).toBe("allow");
  });

  it("still mints when the market maker quotes in-band", () => {
    const rt = boot();
    const { desk, mm } = economy(rt);
    const rfq = must(
      rt.dispatch(
  ...
  it("still names fx_party first when a vendor is off-band while a maker sits", () => {
    const rt = boot();
    const { desk, vendor, mm } = economy(rt);
    const rfq = must(
      rt.dispatch(
  ...
  it("still names payout_fresh first when a vendor conversion pays nothing off-band", () => {
    const { rt, desk, vendor } = emptyPit();
    const rfq = must(
      rt.dispatch(cmd("market.rfq", desk.id, { sku: "fx.usd_sim.usdc_sim", spec: "window" })),
      "fx rfq",
  ...
  it("still names fx_fresh first when a vendor quotes a dead off-band window", () => {
    const { rt, desk, vendor } = emptyPit();
    const rfq = must(
      rt.dispatch(cmd("market.rfq", desk.id, { sku: "fx.usd_sim.usdc_sim", spec: "window" })),
      "fx rfq",
  ...
  it("still names fx_pair first when a vendor quotes a swapped pair off-band", () => {
    const { rt, desk, vendor } = emptyPit();
    const rfq = must(
      rt.dispatch(cmd("market.rfq", desk.id, { sku: "fx.usd_sim.usdc_sim", spec: "window" })),
      "fx rfq",
  ...
  it("still names fx_window first when an FX SKU has no window", () => {
    const { rt, desk, vendor } = emptyPit();
    const rfq = must(
      rt.dispatch(cmd("market.rfq", desk.id, { sku: "fx.usd_sim.usdc_sim", spec: "window" })),
      "fx rfq",
  ...
  it("still names known_rfq first when the RFQ is a ghost and the rate is off-band", () => {
    const { rt, vendor } = emptyPit();
    const before = rt.quotes.size;
    const r = rt.dispatch(
      cmd("market.quote", vendor.id, {
        rfqId: "rfq_01J6AETHERGHOSTFXBAND00001",
        price: { amount: 80_000, currency: "USD_SIM" },
        fx: {
          from: "USD_SIM",
          to: "USDC_SIM",
  ...
  it("still names invited_seller first when an uninvited vendor is off-band", () => {
    const { rt, desk, vendor, compute } = emptyPit();
    const rfq = must(
      rt.dispatch(
        cmd("market.rfq", desk.id, {
          sku: "fx.usd_sim.usdc_sim",
          spec: "window",
          invitedSellerIds: [compute.id],
        }),
      ),
      "fx rfq",
    );
